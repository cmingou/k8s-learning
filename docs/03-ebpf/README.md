...
**實務建議:做 CO-RE 與現代開發,以核心 **5.4+**(理想 5.8+)且**啟用 BTF**(`CONFIG_DEBUG_INFO_BTF=y`)為基準。**(截至 2026 年 9 月,主線核心已進入 **7.x** 系列——[Linux 7.0 於 2026 年 4 月發布](https://kernelnewbies.org/Linux_7.0),其後 7.1 接續,**7.2**(代號 "Baby Opossum Posse")已於 **2026-08-16** 發布([Phoronix 報導](https://www.phoronix.com/news/Linux-7.2-Released)),穩定分支已滾動到 **7.2.7**;主線開發則已進入 **7.3** 的 release candidate 階段(**7.3-rc4** 於 **2026-09-20** 釋出,[Phoronix 報導](https://www.phoronix.com/news/Linux-7.3-rc4)),預計 2026 年 10 月下旬定案;上述 5.4+/5.8+ 只是「CO-RE 可用」的**最低**基準,新專案沒有理由不用更新的 LTS 核心。)

> **近期已知的 eBPF 相關核心安全公告**(提醒:eBPF 的攻擊面包含驗證器、maps、helper 三處,以下各對應一處):
> - **[CVE-2026-63830](https://ubuntu.com/security/CVE-2026-63830)**(CVSS 9.4,Critical,2026-07-19 揭露):`sk_msg` 的 `sg.copy` bitmap 是 scatterlist entry 的「所有權狀態」標記,但 sockmap/TLS 的 transform 路徑在 move、copy、split、compact `msg->sg.data[]` entries 時,沒有同步搬動對應的 `sg.copy` bit,導致外部(page cache)所擁有的記憶體頁被誤判為可由 BPF 修改,可寫入原本唯讀的 page cache 內容。屬於本節分類中的 **maps/helper** 攻擊面;修補見核心 commit [`406e8a651a7b`](https://git.kernel.org/linus/406e8a651a7b854c41fecd5117bb282b3a6c2c6b)。(註:CVSS 9.4 曾是本清單一度最高分,但下面 2026-08-22 揭露的 CVE-2026-74612 以 CVSS 10.0 超越,詳見下方該筆說明。)
...
>
> 2026-09-22 審查後補充:交叉比對 Ubuntu Security Notices 逐一 CVE 編號後發現,2026-06-24 這批公告實際收錄的 BPF 相關 CVE 遠不只上面 8 筆,以下 15 筆先前遺漏,一併補上(其中 CVE-2026-53037/53093/53097 等鄰近編號經查證為同日公告但**非** BPF 子系統問題——分別是 HID `usbhid`、`brcmfmac` WiFi 驅動、`mt76` WiFi 驅動——故不收錄於此):
>
> - **[CVE-2026-53032](https://ubuntu.com/security/CVE-2026-53032)**(CVSS 5.5,Ubuntu priority Medium,2026-06-24 揭露):`map_kptr_match_type()` 原本的檢查順序是先判斷 `btf_is_kernel(reg->btf)` 才檢查 `base_type(reg->type)`,但一個被存入 kptr 欄位的**純量**暫存器根本沒有 `btf`(其 `reg->btf` 為 NULL),導致 `btf_is_kernel()` 對 NULL 解參考崩潰。問題由重構 `bpf_kptr_xchg()` 本地 kptr 邏輯的 commit `ab6c637ad027` 引入。修補把「`base_type(reg->type) != PTR_TO_BTF_ID` 就直接判定不符」的檢查提到任何 `reg->btf` 存取之前。屬於**驗證器**對暫存器型別檢查順序出錯的案例,與上面 CVE-2026-53095(kprobe/freplace 附掛檢查)同屬驗證器檢查邏輯出錯但情境不同的問題。
> - **[CVE-2026-53069](https://ubuntu.com/security/CVE-2026-53069)**(CVSS 7.5,2026-06-24 揭露,syzkaller 回報):`xdp_master_redirect()` 呼叫 bonding 驅動的 `bond_xdp_get_xmit_slave()` → `bond_xdp_xmit_roundrobin_slave_get()` → `bond_rr_gen_slave_id()`,而後者會解參考 `bond->rr_tx_counter`——這個 per-CPU 計數器只有在 `bond_open()`(即介面被 `IFF_UP`)時才會配置。但 `bpf_master_redirect_enabled_key` 是**全域**的 static key,只要系統上**任一顆** bond 裝置掛了原生 XDP,這條 `XDP_TX → xdp_master_redirect()` 的攔截路徑就會對所有 bond slave 全域生效——即使某顆從未 `ifup` 的 bond 裝置,只要收到 XDP_TX 流量就會走進這條路徑,對還沒配置的 `rr_tx_counter` NULL 解參考,造成核心崩潰。修補在呼叫 master 的 `->ndo_xdp_get_xmit_slave()` 前先檢查 `IFF_UP`,不符就以 `XDP_ABORTED` 直接丟棄封包。屬於本節分類中的 **maps/helper**(XDP 導向路徑)攻擊面,與下面 6.1 節已列的 CVE-2026-64545(同一函式 `xdp_master_redirect()` 另一種 NULL 檢查缺漏)同屬一類問題;此案 CVSS 向量為 `AV:N`(可由網路上收到的 XDP 流量觸發,不需要本機權限),是本節少見的「遠端可觸發」BPF 相關 CVE。
> - **[CVE-2026-53074](https://ubuntu.com/security/CVE-2026-53074)**(CVSS 5.5,2026-06-24 揭露):`bpf_prog_test_run_skb()`(供 `BPF_PROG_TEST_RUN` 系統呼叫測試 skb 型程式用)先呼叫 `eth_type_trans()` 判斷協定,再依 `skb->protocol` 存取 `ip_hdr(skb)`/`ipv6_hdr(skb)` 來初始化測試用的 socket family/address 欄位——但若呼叫端只提供了一個 Ethernet header、EtherType 宣稱是 IPv4/IPv6 卻沒有附上實際的 L3 header,就會讀到 skb 資料緩衝區之外的記憶體。修補把 IPv4/IPv6 的最小長度檢查併入既有的協定 switch,長度不足就提早以 `-EINVAL` 拒絕,不再存取網路層 header。屬於本節分類中的 **maps/helper**(`BPF_PROG_TEST_RUN` 測試基礎設施)攻擊面。
> - **[CVE-2026-53076](https://ubuntu.com/security/CVE-2026-53076)**(CVSS 7.1,2026-06-24 揭露):`pcpu_init_value()` 在把資料從一個 map 複製到 per-CPU map 時,假設所有來源 value 都已被核心向上取整到 8 bytes,因此直接呼叫 `copy_map_value_long()` 做整段複製——但 `BPF_MAP_TYPE_CGROUP_STORAGE` 等來源 map 的 `value_size` 若未對齊 8 bytes(例如剛好 4 bytes),驗證器允許的存取範圍就是精確的 4 bytes,而非核取整後的 8 bytes。當這樣的 4-byte 來源被複製進一個 `value_size` 同為 4 bytes 的 per-CPU map 時,`pcpu_init_value()` 仍依 8-byte 假設去讀,造成越界讀取(OOB read)。屬於本節分類中的 **maps/helper** 攻擊面,與上面 CVE-2026-93174(per-CPU map value 複製長度未對齊)同屬「per-CPU map 複製路徑對齊/長度假設出錯」的同類問題家族,只是這次是讀入而非寫出的方向。
> - **[CVE-2026-53078](https://ubuntu.com/security/CVE-2026-53078)**(CVSS 7.8,2026-06-24 揭露):BPF `sock_ops` 程式存取 ctx 欄位時,若 `dst_reg == src_reg`(同一暫存器既是目的也是來源),`SOCK_OPS_GET_SK()`/`SOCK_OPS_GET_FIELD()` 這兩個 codegen 巨集會借用一個暫存暫存器去檢查 `is_fullsock`/`is_locked_tcp_sock`,檢查結果為否(例如 socket 處於 `TCP_NEW_SYN_RECV`、還只是個 `request_sock`)時,理應把 `dst_reg` 清零,但漏了這一步,讓它殘留原本的 ctx 指標值。`SOCK_OPS_GET_SK` 情境下,這個殘留值會被誤判為合法的 `PTR_TO_SOCKET_OR_NULL`,可被當成偽造的 socket 指標傳給 `bpf_skc_to_tcp6_sock()` 等 helper,造成 stack out-of-bounds 存取;`SOCK_OPS_GET_FIELD` 情境下,驗證器則會把這個殘留的核心指標誤判為 `SCALAR_VALUE` 洩漏給使用者空間。修補在 `!fullsock`/`!locked_tcp_sock` 路徑補上 `BPF_MOV64_IMM(dst_reg, 0)`。屬於本節分類中的 **maps/helper**(ctx 欄位存取 codegen)攻擊面,是少見的「同暫存器 dst/src」邊界情況被漏掉的案例。
> - **[CVE-2026-53083](https://ubuntu.com/security/CVE-2026-53083)**(CVSS 5.5,2026-06-24 揭露):`bpf_fd_array_map_clear()` 清空 `PROG_ARRAY` map 時,對每個 entry 都呼叫一次可能很昂貴的 `prog_array_map_poke_run()`,但迴圈中沒有 `cond_resched()`。當 map 的 entry 數量很大時,這個長迴圈會長時間佔用 CPU 不釋放,導致 RCU 寬限期遲遲無法結束,在高負載下觸發 RCU stall(核心回報 `rcu: INFO: rcu_sched detected stalls`),使系統回呼與 workqueue 大幅延遲,形同阻斷服務。修補在迴圈中補上 `cond_resched()`。屬於本節分類中的 **maps/helper**(map 生命週期)攻擊面。
> - **[CVE-2026-53084](https://ubuntu.com/security/CVE-2026-53084)**(CVSS 5.5,2026-06-24 揭露):與下一筆 CVE-2026-53085 同屬 `task_vma` iterator 系列問題,但錯在鎖順序而非參照計數。iterator 原本在**整個** BPF 程式主體執行期間持有 per-VMA 鎖(`vm_lock`),但若程式呼叫的 helper 內部又需要取得依賴 `mmap_lock` 的鎖(鎖順序為 `vm_lock → i_rwsem → mmap_lock → vm_lock`),就會形成鎖順序反轉,可能導致死鎖。修補改為在 `_next()` 內、仍持有 per-VMA 鎖時就用 `memcpy()` 把 VMA 資料**快照**下來,再釋放鎖後才把快照交給 BPF 程式本體使用(驗證器只信任快照中的 `vm_mm`/`vm_file` 指標,`vm_file` 另外用 `get_file()` 額外取得參照、於下次迭代或 `_destroy()` 以 `fput()` 釋放)。修補見核心 commit [`4cbee026db54`](https://github.com/torvalds/linux/commit/4cbee026db54)("bpf: return VMA snapshot from task_vma iterator")。屬於本節分類中的 **maps/helper**(iterator)攻擊面。
> - **[CVE-2026-53089](https://ubuntu.com/security/CVE-2026-53089)**(CVSS 7.8,2026-06-24 揭露):查詢已 offload 到硬體的 BPF map/程式資訊時,`bpf_map_offload_info_fill_ns()`/`bpf_prog_offload_info_fill_ns()` 用 `get_net(dev_net(offmap->netdev))` 取得對應的 network namespace 參照——`rtnl_lock`/`bpf_devs_lock` 確保 `netdev` 指標本身有效,但無法阻止該 netns 的參照計數在同一時間因 netns 銷毀而歸零。一旦 `get_net()` 在參照計數已經是 0 的情況下仍執行遞增,就會觸發 `refcount_t: addition on 0`,屬於 Use-After-Free 的一種形式。修補改用 `maybe_get_net()`(內部用 `refcount_inc_not_zero()`),計數已歸零時回傳 `NULL`,讓呼叫端改以 `-ENOENT` 正常失敗而非毀損記憶體。修補見核心 commit [`a0c584fc1805`](https://github.com/torvalds/linux/commit/a0c584fc18056709c8e047a82a6045d6c209f4ce)。屬於本節分類中的 **maps/helper**(BPF offload)攻擊面。
> - **[CVE-2026-53092](https://ubuntu.com/security/CVE-2026-53092)**(CVSS 7.8,Ubuntu priority Low,2026-06-24 揭露):驗證器 `adjust_reg_min_max_vals()` 處理 `rX += rX`(即 `src_reg`、`dst_reg` 指向同一個 `bpf_reg_state`)這種邊界情況時,函式先就地修改了 `dst_reg` 的值,但後續計算 delta(用於 `sync_linked_regs()` 把邊界變化同步給其他「連動 (linked)」暫存器)時,讀取 `src_reg` 拿到的卻是**修改後**的值而非原始來源值,導致算出錯誤的 delta,被 `sync_linked_regs()` 錯誤傳播到其他連動暫存器上,形成驗證器推導與執行期實際值不一致。修補直接跳過 `src_reg == dst_reg` 這個特例。修補見核心 commit [`cc86a8b0a1c5`](https://github.com/torvalds/linux/commit/cc86a8b0a1c54d2bccf6f68cf49b82dea91b84de)("bpf: Fix linked reg delta tracking when src_reg == dst_reg")。屬於**驗證器**(scalar/連動暫存器追蹤)層級的問題,與上面 CVE-2026-93127(窄讀取正負號擴展與 id 共用衝突)、CVE-2026-53081(`BPF_ADD_CONST` 誤用基底 id)同屬驗證器對純量追蹤處理出錯的同類問題家族。
> - **[CVE-2026-53094](https://ubuntu.com/security/CVE-2026-53094)**(CVSS 7.8,Ubuntu priority Low,2026-06-24 揭露):`BPF_F_XDP_DEV_BOUND_ONLY`(裝置綁定 (dev-bound-only) XDP 程式,offload 給網卡硬體)在啟用**常數混淆 (constant blinding)**(`bpf_jit_harden >= 2`)做 JIT 編譯時,`bpf_jit_blind_constants()` 會複製 (clone) 出一份新程式,原始程式隨後在 `bpf_jit_prog_release_other()` 中被釋放——這個函式會更新 `aux->prog` 指向存活下來的複製品,但漏了同步更新 `offload->prog`,讓它繼續指向已被釋放的原始程式。之後 network namespace 被銷毀、觸發 `bpf_dev_bound_netdev_unregister()` 走訪這個懸空的 `offload->prog` 時,即形成 Use-After-Free。修補讓 `bpf_jit_prog_release_other()` 同步更新 `offload->prog`。屬於本節分類中的 **maps/helper**(BPF offload/JIT 常數混淆)攻擊面。
> - **[CVE-2026-53096](https://ubuntu.com/security/CVE-2026-53096)**(CVSS 7.8,Ubuntu priority Low,2026-06-24 揭露):`dev_map_redirect_multi()`(`BPF_MAP_TYPE_DEVMAP_HASH` 的多播轉送路徑,由 softirq context 的 `xdp_do_generic_redirect_map()` 呼叫、跑在 RCU read-side critical section 內)走訪 hash bucket 時用了 `hlist_for_each_entry_safe()`,而非 RCU-safe 的 `hlist_for_each_entry_rcu()`;但併發的寫入端(`__dev_map_hash_update_elem()`、`dev_map_hash_delete_elem()`)是用 `hlist_add_head_rcu()`/`hlist_del_rcu()` 等 RCU 原語修改同一份 list。`hlist_for_each_entry_safe()` 做的是一般的指標解參考,缺少與寫入端配對所需的 acquire barrier,可能讀到修改中、不一致的 list 結構,造成記憶體毀損或崩潰。屬於本節分類中的 **maps/helper**(devmap XDP 多播轉送)攻擊面,與上面 CVE-2026-53069、6.1 節 CVE-2026-64545 同屬 XDP 導向 (redirect) 路徑出錯的系列問題,只是這次錯在 RCU 走訪方式而非 NULL 檢查。
> - **[CVE-2026-53099](https://ubuntu.com/security/CVE-2026-53099)**(CVSS 5.5,Ubuntu priority Low,2026-06-24 揭露):較早的重構把編譯選項 `CONFIG_CFI_CLANG` 改名為編譯器無關的 `CONFIG_CFI`(commit `23ef9d439769`),但 BPF 子系統中用來保護 `btf_dtor_kfunc_t`(BTF 解構子 kfunc 指標)的 kCFI(Control Flow Integrity,比對函式指標實際型別與呼叫端預期型別)相關程式碼,仍沿用舊的 `CONFIG_CFI_CLANG` 名稱做條件編譯——名稱對不上,導致這段原本該啟用的 CFI 保護程式碼實際上被編譯器整段排除,`btf_dtor_kfunc_t` 的型別混淆 (type confusion) 攻擊面因此失去 kCFI 保護。屬於**驗證器/核心介接層**對 BPF callback 型別檢查失效的案例,與上面 CVE-2026-92486(BPF callback ABI 與 kCFI 型別不一致觸發 Panic)同屬 kCFI 相關的同類問題,但這次是保護機制本身因設定名稱過期而失效,而非型別本身不一致。
> - **[CVE-2026-53106](https://ubuntu.com/security/CVE-2026-53106)**(CVSS 5.5,2026-06-24 揭露):BPF local storage(`bpf_xxx_storage_delete()` 系列 helper)刪除 selem 時,可能透過 `kfree_rcu()`、`call_rcu()` 或 `call_rcu_tasks_trace()` 延遲釋放——但這些延遲釋放路徑在 **NMI** 或可重入 (reentrant) context 下呼叫可能造成死鎖。修補先針對「在 NMI 中呼叫」這個明確可辨識的情境,讓 `bpf_xxx_storage_delete()` 系列 helper 直接回傳錯誤而不嘗試刪除(commit message 也坦言:可重入情境下的死鎖尚未完全解決,是長期待解的問題)。屬於本節分類中的 **maps/helper**(local storage)攻擊面。
> - **[CVE-2026-53110](https://ubuntu.com/security/CVE-2026-53110)**(CVSS 7.8,Ubuntu priority Low,2026-06-24 揭露):s390x ABI 規定呼叫端必須把無號引數做零擴展 (zero-extend)、有號引數做正負號擴展 (sign-extend),被呼叫端(callee)對回傳值也要遵守同樣規則——但 s390 的 BPF JIT 編譯器只實作了正負號擴展,漏了零擴展這一半。這讓 BPF 程式的回傳值或 kfunc 引數在暫存器高位殘留未定義的舊資料,可能被後續程式碼誤讀成錯誤的數值,影響控制流或記憶體存取正確性。修補為 s390 JIT 補上零擴展邏輯。修補見核心 commit [`202e42e4aa89`](https://github.com/torvalds/linux/commit/202e42e4aa89)("s390/bpf: Zero-extend bpf prog return values and kfunc arguments")。屬於 **JIT 編譯器**層級的 ABI 合規性出錯,與上面 CVE-2026-89581(x86 JIT REX prefix 誤用)、CVE-2026-93147(s390 JIT 誤用 `ly` 而非 `llgf`)同屬架構特定的 JIT 編譯器錯誤,這次是 s390 上零擴展規則整個被遺漏,而非單一指令選錯。
> - **[CVE-2026-53111](https://ubuntu.com/security/CVE-2026-53111)**(CVSS 5.5,2026-06-24 揭露):`bpf_lwt_xmit_push_encap()` 這個 helper 需要讀取 `skb_dst(skb)->dev` 來計算封裝所需的 headroom,但透過 `bpf_prog_test_run_skb()`(`BPF_PROG_TEST_RUN`)建構的測試用 skb,其 `_skb_refdst` 欄位可能根本沒有被初始化,導致 `skb_dst(skb)` 回傳的 dst entry 為 NULL,對 `->dev` 解參考時觸發 NULL pointer dereference、核心崩潰。屬於本節分類中的 **maps/helper**(`BPF_PROG_TEST_RUN` 測試基礎設施)攻擊面,與上面 CVE-2026-53074(同樣是 `bpf_prog_test_run_skb` 測試路徑對輸入假設不足)同屬測試基礎設施對輸入前提檢查不足的同類問題。
> 上述 **CVE-2026-53031/53032/53033/53034/53035/53036/53069/53074/53076/53078/53081/53083/53084/53085/53089/53090/53092/53094/53096/53099/53106/53110/53111** 與下面的 **CVE-2026-53095** 合計 24 筆,同屬 2026-06-24 同一批公告的核心 BPF 修補,是本清單收錄數量最多的單一批次,建議一併留意。
...
> - **[CVE-2026-64354](https://ubuntu.com/security/CVE-2026-64354)**(CVSS 7.8 High,Ubuntu priority Medium,2026-07-25 揭露,2026-09-25 系統性複查時發現先前遺漏、補上):`BPF_BTF_LOAD` 載入使用者提供的 BTF 資料時,`btf_parse_struct_metas()` 用 `btf_repeat_fields()` 以 `u32` 運算計算展開後的欄位數量——構造過的惡意 BTF 可讓這個計算整數溢位繞回一個很小的值,使原本該擋下「展開欄位數超出暫存陣列容量」的檢查誤判通過,後續 `memcpy()` 便對一段核心堆疊上、大小固定為 `BTF_FIELDS_MAX` 的暫存陣列寫入超出邊界的資料,造成 stack buffer overflow。修補在複製欄位前加上溢位檢查,拒絕不合理的欄位數量。修補見核心 commit [`c5ff816d5f13`](https://git.kernel.org/stable/c/c5ff816d5f13900c3f1f3298cfcc61339e056e56)。屬於本節分類中的 **maps/helper**(BTF 載入)攻擊面,與上面 CVE-2026-64548(sockmap `copy + len` u32 溢位)、CVE-2026-53076(per-CPU 對齊假設導致越界)同屬「邊界檢查用的整數運算本身可被溢位繞過」的同類問題。
...
> - **[CVE-2026-72402](https://ubuntu.com/security/CVE-2026-72402)**(CVSS 5.5 Medium,Ubuntu priority Medium,2026-08-15 揭露,同批公告,2026-09-22 審查後補充):`print_bpf_insn()` 在驗證器記錄檔(verifier log)中列印 `ldimm64` 指令時,會遮蔽 (mask) 帶有指標的偽來源 (pseudo source) 立即值,避免核心指標位址外洩到記錄檔——但原本的遮蔽邏輯只涵蓋 `BPF_PSEUDO_MAP_FD`、`BPF_PSEUDO_MAP_VALUE` 這兩種偽來源型別,漏掉了同樣會在列印前被解析成核心指標值的 `BPF_PSEUDO_MAP_IDX`、`BPF_PSEUDO_MAP_IDX_VALUE`、`BPF_PSEUDO_BTF_ID`。持有可讀取驗證器記錄檔權限(例如透過 `BPF_PROG_LOAD` 的 `log_buf` 或核心記錄)的本機使用者,可藉此取得原本應被遮蔽的核心指標位址,作為繞過 KASLR 等後續攻擊的線索(CWE-117,Improper Logging)。修補把這三種偽來源型別一併納入遮蔽邏輯的判斷式,讓記錄檔統一印出 `0x0`。修補見核心 commit [`72a85e9464a5`](https://github.com/torvalds/linux/commit/72a85e9464a5332fb2cd7efd26d9295275ceda2d)("bpf: Mask pseudo pointer values in verifier logs when pointer leak is not allowed")。屬於**驗證器**記錄檔資訊外洩的案例,與上面 6.1 節 CVE-2026-52910、CVE-2026-92520(未清零的核心堆疊內容外洩給使用者空間)同屬「核心內部資訊(指標位址、未初始化記憶體)不當外洩」的同類問題,但這次外洩管道是驗證器 debug 記錄檔,而非 map value 或 socket 選核結果。
...
> - **[CVE-2026-89516](https://ubuntu.com/security/CVE-2026-89516)**(CVSS 未公布,Ubuntu priority Medium,2026-09-11 揭露,與上面一筆同日公告):`sched_ext` dispatch 路徑上的 `process_deferred_reenq_users()` 在處理延遲重新排入佇列(deferred reenqueue)請求時,若對應的 DSQ(Dispatch Sequence Queue)在 `scx_bpf_dsq_reenq()` 佇列請求送出後、`run_deferred()` 真正執行前就已被銷毀,函式會直接對這個已銷毀的 DSQ 執行 `BUG_ON()` 斷言,造成核心崩潰(阻斷服務)。修補改用 `READ_ONCE()` 讀取 DSQ 狀態,發現已銷毀就跳過該筆延遲請求,而不是斷言崩潰。修補見核心 commit [`8d8dd8ae89ea`](https://github.com/torvalds/linux/commit/8d8dd8ae89eaa78b37fc85528e926029f5facbdf)("sched_ext: Don't BUG_ON a destroyed DSQ in process_deferred_reenq_users")。屬於本節分類中的 **maps/helper**(sched_ext kfunc)攻擊面,與下面同批(2026-09-11)公告的 CVE-2026-89518 同屬 `sched_ext`/`BPF_PROG_TYPE_STRUCT_OPS` 可插拔排程器 dispatch 路徑出錯的姊妹案例,錯在 DSQ 銷毀時機競態,而非 rq 誤判;主流 Ubuntu LTS 版本目前均標示「Not affected」。
>
> 上述 **CVE-2026-89564/89516** 為 2026-09-11 公告的同一批修補。
> - **[CVE-2026-89579](https://ubuntu.com/security/CVE-2026-89579)**(CVSS 7.8 High,Ubuntu priority Medium,2026-09-11 揭露):BPF bloom filter map(`BPF_MAP_TYPE_BLOOM_FILTER`)在**32 位元**核心上的 `bloom_map_alloc()`,計算所需 bitset 大小時呼叫 `BITS_TO_BYTES(U32_MAX)`——這段用的是 32 位元運算,`DIV_ROUND_UP` 內部的加法在數值逼近 `U32_MAX` 時發生整數溢位而回捲,實際配置到的記憶體只有一個固定大小的 bloom filter 物件,但用來檢查存取範圍的 `bitset_mask` 欄位卻仍被設成溢位前的 `U32_MAX`,兩者完全對不上。更麻煩的是雜湊值算出來是 `u32`,但核心的 `set_bit()` 接受的是**帶正負號**的 `long` 偏移量——在 32 位元系統上,只要雜湊值落在 `0x80000000` 到 `U32_MAX` 之間就會被當成負偏移量,直接對配置範圍外的記憶體讀寫。持有 `CAP_BPF` 的本機使用者可藉由觸發這個「配置不足 + 負偏移量」的組合,在 32 位元 x86 核心上做到本機權限提升。修補改為把雜湊值拆成「字組指標」與「字組內位元編號」兩部分後再分別呼叫 bit 操作函式,確保偏移量永遠落在合法範圍內。修補見核心 commit [`11c1e83`](https://git.kernel.org/stable/c/11c1e836710dcba03e50454a4eedfdbaf8d3050e)。屬於本節分類中的 **maps/helper**(bloom filter map,僅影響 32 位元核心)攻擊面,是少見的「架構特定 (32-bit only)」本機權限提升案例。
> - **[CVE-2026-89580](https://ubuntu.com/security/CVE-2026-89580)**(CVSS 7.8 High,Ubuntu priority Medium,2026-09-11 揭露):`get_perf_callchain()`(供 `bpf_get_stack()`/`bpf_get_stackid()` 等 helper 使用)會回傳一段 per-CPU 的 `perf_callchain_entry` 緩衝區,並在回傳**之前**就先透過 `put_callchain_entry()` 釋放它的遞迴保護槽——等於在 `__bpf_get_stack()` 真正把資料複製出來之前,已經沒有任何機制保住這段緩衝區不被其他執行緒搶走。在**可搶佔 (preemptible)** 的核心組態下(例如掛在非睡眠 raw tracepoint 上、僅靠 `migrate_disable()` 而非 `preempt_disable()` 保護的 BPF 程式),「取得緩衝區」與「複製資料」之間的空窗可能被搶佔,同一顆 CPU 上另一個任務重用同一塊 per-CPU 緩衝區並覆寫 `trace->nr`,造成後續 `memcpy()` 依這個失真的長度值寫出緩衝區邊界(out-of-bounds write)。修補改為在取得緩衝區、複製資料的整段期間關閉搶佔,可能觸發 page fault 的 build-id 解析則延後到重新開啟搶佔之後、只對私有副本操作。問題可追溯至 2018 年引入 `bpf_get_stack` helper 的 commit `c195651e565a`,修補見核心 commit [`15f1bd857466`](https://github.com/torvalds/linux/commit/15f1bd8574662f1b7b26aaa2e23ebf4066f0117d)("bpf: Disable preemption in bpf_get_stackid")。屬於本節分類中的 **maps/helper**(棧回溯 stack-unwinding helper)攻擊面。
> - **[CVE-2026-89581](https://ubuntu.com/security/CVE-2026-89581)**(CVSS 7.8 High,Ubuntu priority Medium,2026-09-11 揭露):x86 BPF JIT 編譯器在組譯存取 per-CPU 變數位址的指令時,目的暫存器是編碼在 `ModRM.reg`、需要 `REX.R` 位元才能定址到擴充暫存器(R8-R15),但組譯 REX prefix 時誤用了 `add_1mod()`(只會設定 `REX.B`,用於定址 `ModRM.rm`/`SIB.base`)而非正確的 `add_2mod()`;由於這段指令定址模式是「`disp32` 且無 base 暫存器」,`REX.B` 在此完全不起作用,導致高位暫存器位元直接遺失。實際效應是所有 `is_ereg()` 判定為擴充暫存器的目的位址,都會解析成「共用相同低三位元」的錯誤暫存器(例如 R5→RAX、R7→RBP、R8→RSI、R9→RDI),使 BPF 程式讀寫 per-CPU 變數時,實際存取到完全不相干的暫存器內容,可能造成記憶體毀損或核心 Panic。此問題自 **6.10** 引入(commit `7bdbf7446305`)後長期潛伏——用 Clang 編譯的 BPF 程式因為每次存取前都會重新載入位址,不會踩到這個路徑,直到 GCC 編譯的 BPF 程式(會讓多個 per-CPU 位址同時存活於暫存器中)在 `test_progs-bpf_gcc` 測試中促發才被發現。修補把 `add_1mod()` 改為 `add_2mod()`,已於穩定版 **6.12.109 / 6.18.50 / 7.2.4 / 7.3-rc1** 修補,此處連結為 6.12 穩定分支的對應修補 commit [`638bc3aada8e`](https://github.com/torvalds/linux/commit/638bc3aada8ecdece184d5c15b100d489c9cccd7)。屬於 **JIT 編譯器**層級的指令編碼出錯,與上面 6.1 節 arm64 的 CVE-2026-53036(分支位移邊界檢查出錯)同屬 JIT 編譯器出錯的案例,但這次錯在暫存器編碼而非邊界檢查。
> - **[CVE-2026-89518](https://ubuntu.com/security/CVE-2026-89518)**(CVSS 未公布,Ubuntu priority Medium,2026-09-11 揭露,與上面三筆同批公告):`sched_ext`(以 `BPF_PROG_TYPE_STRUCT_OPS` 實作、可用 BPF 程式客製化排程邏輯的可插拔排程器類別)dispatch 路徑上的多個 kfunc——`scx_dsq_move()`、`scx_bpf_sub_dispatch()`、`finish_dispatch()`、`scx_bpf_dsq_reenq()`、`scx_bpf_dsq_nr_queued()`——原本都假設「目前執行的 CPU」與「被 dispatch 目標的 run queue (rq)」永遠相同;但在核心排程 (core scheduling) 下,dispatch 可能於 core-wide pick 過程中對兄弟 (sibling) rq 執行,使 `this_rq()` 與實際 dispatch 目標不一致——`scx_dsq_move()` 可能因此誤判鎖狀態,對已持有鎖的目標 rq 重複上鎖而死鎖;`finish_dispatch()`/`scx_bpf_dsq_reenq()`/`scx_bpf_dsq_nr_queued()` 則可能把 `SCX_DSQ_LOCAL` 解析成執行緒目前所在 CPU 的本地 DSQ,而非真正被 dispatch 的 rq 所屬 DSQ。修補改為統一使用 `scx_locked_rq()`(在 ops 呼叫期間指向真正被 dispatch 的 rq,非持鎖情境下為 `NULL`)取代 `this_rq()`。修補見核心 commit [`3dd52416e44a`](https://git.kernel.org/linus/3dd52416e44a70bc993adb96d2e0d71b9ea21359)。屬於本節分類中的 **maps/helper**(sched_ext kfunc)攻擊面,是本節收錄清單中少見的、與 `BPF_PROG_TYPE_STRUCT_OPS`/`sched_ext` 可插拔排程器相關而非傳統網路/追蹤路徑的案例,提醒 BPF 的攻擊面也涵蓋以 struct_ops 形式暴露給 BPF 程式的核心子系統;與上面 2026-09-11 揭露的 CVE-2026-89516 同屬 `sched_ext` dispatch 路徑出錯的姊妹案例,只是這次錯在 rq 誤判而非 DSQ 銷毀時機競態。
>
> 上述 **CVE-2026-89579/89580/89581/89518** 為 2026-09-11 公告的同一批修補,與上面的 CVE-2026-89564/89516 同日但分開兩批公告。
> - **[CVE-2026-90001](https://ubuntu.com/security/CVE-2026-90001)**(CVSS 7.8 High,2026-09-17 揭露):**HID-BPF**(讓 BPF 程式客製化 HID(Human Interface Device,如滑鼠、鍵盤、觸控板)裝置行為的框架,自核心 6.10 起改以 `BPF_PROG_TYPE_STRUCT_OPS` 實作,見 commit `ebc0d80`「HID: bpf: implement HID-BPF through bpf_struct_ops」)的 struct_ops 銷毀路徑中,裝置移除路徑 `__hid_bpf_ops_destroy_device()` 與 BPF link 釋放路徑 `hid_bpf_unreg()` 分別在不同的鎖定範圍下,各自用「`ops->hdev` 是否為 NULL」判斷該不該由自己釋放裝置參照——但 `hid_bpf_unreg()` 可能先讀到 `ops->hdev` 非 NULL、卡在鎖上等待時,銷毀路徑已先把它清空並釋放參照,導致兩條路徑對同一筆裝置參照各自 double-put,提前釋放 `struct hid_device` 造成 Use-After-Free。修補統一在 `prog_list_lock` 底下做「該不該清空/釋放」的判斷:銷毀路徑改為持鎖期間先計數、鬆鎖後才真正釋放參照;unreg 路徑則在鎖內重新檢查 `ops->hdev`,若已被清空就直接提早返回。修補見核心 commit [`9cdc7e6`](https://github.com/torvalds/linux/commit/9cdc7e6dc7a99ad7311ad5e7c145f2b9ce4e24b0)("HID: bpf: serialize device reference release in struct_ops destroy path")。屬於本節分類中的 **maps/helper**(`BPF_PROG_TYPE_STRUCT_OPS`)攻擊面,與上面 2026-09-11 揭露的 CVE-2026-89518、CVE-2026-89516 同屬 struct_ops 生命週期/併發管理出錯的同類問題,只是這次錯在 HID-BPF 裝置銷毀路徑的參照計數競態,而非 sched_ext 排程 dispatch 路徑。
> - **[CVE-2026-93043](https://raw.githubusercontent.com/CVEProject/cvelistV5/main/cves/2026/93xxx/CVE-2026-93043.json)**(CVSS 未公布,尚未列入 Ubuntu Security Notices,2026-09-17 揭露):BPF 直譯器 (interpreter) 不認得 `insn_array` map(間接跳躍/「gotox」機制)所用的 `BPF_JMP|BPF_JA|BPF_X` 指令——若程式因故(例如核心關閉 JIT,或退回直譯器執行)不走 JIT 而改由直譯器執行,`___bpf_prog_run()` 踩到這個指令會直接觸發 `BUG_ON()`,造成核心 Panic(本機阻斷服務)。修補在驗證階段就檢查程式是否使用了 `insn_array` map,若有就設定 `prog->jit_required = true`,強制要求 JIT、不允許直譯器 fallback,而不是等到執行期才 Panic。修補見核心 commit [`17da80d0c7b5`](https://git.kernel.org/stable/c/17da80d0c7b56579476b6644120bcfea778e531a)、[`905f716362e1`](https://git.kernel.org/stable/c/905f716362e1186c1a23447ca279e6d21f795cdb)。屬於本節分類中的 **maps/helper**(insn_array/gotox)攻擊面,與下面 CVE-2026-93044、CVE-2026-93135 同屬「直譯器不支援某指令卻仍允許 fallback」這一類問題,同批(2026-09-17)公告;截至撰文時 Ubuntu Security Notices 尚未收錄此案,故連結至官方 CVE Program 的原始記錄。
> - **[CVE-2026-93044](https://raw.githubusercontent.com/CVEProject/cvelistV5/main/cves/2026/93xxx/CVE-2026-93044.json)**(CVSS 未公布,尚未列入 Ubuntu Security Notices,2026-09-17 揭露):與上一筆同批修補,問題模式相同但換成 **BPF arena**(`BPF_MAP_TYPE_ARENA`)——直譯器同樣不支援 arena 相關的 ST/LDX/STX 指令,若使用 arena 的程式退回直譯器執行,會在 `__bpf_prog_select_runtime()` 之後踩到「未知 opcode」的核心 BUG。修補同樣是在驗證階段對使用 arena map 的程式設定 `jit_required = true`。修補見核心 commit [`408b6d3bc19c`](https://git.kernel.org/stable/c/408b6d3bc19c4aa852ab7ee2f447756e52e69967)、[`34746b5a84ec`](https://git.kernel.org/stable/c/34746b5a84ec37c0ea2bf6808c65c5ed8790eb51)。屬於本節分類中的 **maps/helper**(arena)攻擊面,與上面 CVE-2026-53031(arena 配置時 `node_id` 未驗證)同屬 arena 相關的姊妹問題,但這次錯在直譯器 fallback,而非邊界檢查。
> - **[CVE-2026-93045](https://ubuntu.com/security/CVE-2026-93045)**(CVSS 7.8 High,2026-09-17 揭露):`bpf_arena_free_pages()` 從 BPF 程式接收一個純量的 arena 位址,先截斷成低 32 位元、再加上 arena base 還原成完整的使用者空間位址——但若這個純量值低於 arena base 的低 32 位元,還原出來的 `full_uaddr` 會落在 `user_vm_start` **之下**,而既有的邊界裁切只檢查了上界、沒檢查下界,讓這個越界位址被當成合法偏移量寫入 arena 的釋放樹 (free-tree)。後續配置若重用到這筆錯誤的偏移量,就可能回傳 arena 實際映射範圍**之外**的頁面,造成攻擊者可影響、映射範圍外的記憶體被讀寫。修補在計算偏移量之前,先擋下位址低於 `user_vm_start` 的釋放請求。修補見核心 commit [`90453eaf8f14`](https://git.kernel.org/stable/c/90453eaf8f14e7be0ab03983f2a93aeb6b990a58)等 4 筆對應穩定分支修補(6.12.110/6.18.52/7.2.6/7.3-rc1)。屬於本節分類中的 **maps/helper**(arena)攻擊面,與上面 CVE-2026-53031 同屬 arena 邊界檢查問題,但這次錯在釋放路徑漏了下界檢查,而非配置路徑的 `node_id`。
> - **[CVE-2026-93135](https://raw.githubusercontent.com/CVEProject/cvelistV5/main/cves/2026/93xxx/CVE-2026-93135.json)**(CVSS 未公布,尚未列入 Ubuntu Security Notices,2026-09-17 揭露):部分架構(核心原始說明點名 LoongArch、ARM64、RISC-V)的 JIT 會把 helper 呼叫直接內嵌 (inline) 進編譯後的指令、並改寫呼叫的位移量——但若當下 JIT 被關閉、或 JIT 編譯本身失敗而動態退回直譯器執行,直譯器執行到的會是這段「已改寫成內嵌位移量、但直譯器根本不懂得怎麼解讀」的指令,可能造成核心 Panic。修補在程式使用了這類 helper 內嵌功能、但 JIT 不可用時直接拒絕載入,而不是讓它退回直譯器。修補見核心 commit [`21c644536679`](https://git.kernel.org/stable/c/21c6445366795fa3a89954e1d318e7c8afdef843)、[`f1c27922576e`](https://git.kernel.org/stable/c/f1c27922576edccb99d0257827d09bd05c0304a6)。屬於本節分類中的 **maps/helper**(helper call inlining)攻擊面,與上面 CVE-2026-93043、CVE-2026-93044 同屬「直譯器不支援某種 JIT-only 最佳化,卻仍允許 fallback」這一類問題,只是這次觸發條件是 helper 呼叫內嵌而非 gotox/arena。
> - **[CVE-2026-93138](https://ubuntu.com/security/CVE-2026-93138)**(CVSS 7.8 High,2026-09-17 揭露):核心用來快取 vmlinux BTF(對應核心自身型別資訊)的全域指標,在 `bpf_get_btf_vmlinux()` 中用一般的變數賦值「發布」出去,之後其他執行緒也只是用一般的無鎖讀取去檢查這個指標是否已設定——兩邊都沒有對應的記憶體屏障,在弱記憶體排序架構上,讀取端可能先看到指標已非 NULL,卻還沒看到指標所指向的 BTF 物件完成初始化的內容,等於用到尚未初始化完整的核心記憶體。修補改用 `smp_store_release()` 發布指標、`smp_load_acquire()` 讀取,確保「看到指標非 NULL」時,其指向的內容也保證已初始化完成。修補見核心 commit [`92863e678070`](https://git.kernel.org/linus/92863e678070f57c17c868e4bfa2441a5c61ad2b)(問題由 `8580ac9` 引入)。屬於**驗證器**(vmlinux BTF 初始化)層級的記憶體排序 (memory ordering) 問題,是本節少見的「發布/讀取端缺屏障」類型案例。
> - **[CVE-2026-90313](https://raw.githubusercontent.com/CVEProject/cvelistV5/main/cves/2026/90xxx/CVE-2026-90313.json)**(CVSS 未公布,尚未列入 Ubuntu Security Notices,2026-09-17 揭露):替換 (replace) cgroup BPF 程式的路徑(`__cgroup_bpf_attach`)在替換過程失敗、需要復原成原本程式時,沒有正確還原原本的 `old_pl_flags`/`old_storage`/`old_flags`,導致原本(未被替換掉)的程式繼續執行、呼叫 `bpf_get_local_storage()` 時,拿到的是已經失效的儲存指標,形成類似 Use-After-Free 的無效記憶體存取。修補改為在嘗試替換之前先保存這些欄位,失敗時完整復原。修補見核心 commit [`e26db0d636e4`](https://git.kernel.org/stable/c/e26db0d636e4c24a6b16683ea95cf5077c64d74b)等對應穩定分支修補(6.6.157/6.12.110/6.18.52/7.2.6/7.3-rc1)。屬於本節分類中的 **maps/helper**(cgroup local storage)攻擊面,是少見的「附掛/替換失敗路徑清理不完整」導致 UAF 的案例。
>
> 上述 **CVE-2026-93043/93044/93045/93135/93138/90313** 為 2026-09-17 公告的一批修補,與上面同日的 CVE-2026-90001 合計本批共 7 筆。
>
> - **[CVE-2026-93144](https://ubuntu.com/security/CVE-2026-93144)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):驗證器 `check_ptr_to_btf_access()` 讓依程式類型客製化的 `btf_struct_access` callback,在核心預設的唯讀存取檢查介入之前就先自行判斷該不該允許寫入——這使得原本應該完全唯讀的 `PTR_UNTRUSTED` BTF 指標(例如透過 `bpf_rdonly_cast()` 取得的指標)得以被寫入,讓 BPF 程式能經由這類指標寫入任意核心記憶體。修補在共同進入點就先擋下對 `PTR_UNTRUSTED` 指標的非讀取操作,不再讓個別 callback 有機會繞過。問題源自 `bpf_obj_new()`/allocated objects 支援引入時的 commit `282de14`。修補見核心 commit [`ac65c710cc64`](https://github.com/torvalds/linux/commit/ac65c710cc643cbc52b899627577357867249530)("bpf: Reject writes through untrusted BTF pointers")。屬於**驗證器**對 BTF 指標唯讀性檢查不足的案例,與下面 CVE-2026-93148(同批公告、同屬 MEM_ALLOC/BTF 物件邊界檢查)屬於同一天揭露的姊妹問題。
> - **[CVE-2026-93147](https://ubuntu.com/security/CVE-2026-93147)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):s390 BPF JIT 編譯器把 32 位元值(`cpu_nr`)載入 64 位元 BPF 暫存器時,誤用了不會零擴展(zero-extend)高位的 `ly` 指令,而非正確應使用的 `llgf`,導致暫存器高 32 位殘留舊資料。修補把 `ly` 換成 `llgf`。問題源自 commit `9012cf2`。修補見核心 commit [`5f6cc299938b`](https://github.com/torvalds/linux/commit/5f6cc299938b561cb01e343bab7042611fcee12a)("s390/bpf: Replace ly instruction with llgf")。屬於 **JIT 編譯器**指令編碼出錯的案例,與上面 CVE-2026-89581(x86 JIT REX prefix 誤用 `add_1mod()`)同屬「JIT 組譯階段選錯指令/prefix 導致暫存器內容失真」的同類問題,只是這次是架構特定於 s390。
> - **[CVE-2026-93148](https://ubuntu.com/security/CVE-2026-93148)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):`PTR_TO_BTF_ID | MEM_ALLOC` 物件(來自 `bpf_obj_new()` 或 percpu kptr)是以固定的 BTF struct 大小配置,但驗證器對 BTF struct walk 原本設計給一般 BTF 型別走訪用的「trailing flexible array 邊界放寬」規則,卻沒有排除這類固定大小配置的物件——導致存取範圍在套用這項放寬之前就先繞過了本該有的 struct-size 邊界檢查,可能讓存取跨出實際配置的物件邊界。修補在對 MEM_ALLOC 物件做 BTF walk 時,先擋下超出 struct 大小的存取範圍,再套用 flexible-array 放寬規則,並把同一套規則同步套用到 struct ID 比對(避免 kfunc/kptr 型別檢查一樣繞過物件邊界)。問題源自 `bpf_obj_new()` 引入的 commit `958cf2e`,以及 percpu kptr 的 alloc/xchg/direct_access 支援 commit `36d8bdf`。修補見核心 commit [`9c9ee0324c77`](https://github.com/torvalds/linux/commit/9c9ee0324c774490ae953162aaaf4561d222bd93)("bpf: Reject MEM_ALLOC BTF accesses past object bounds")。屬於**驗證器**對 allocated objects 邊界檢查不足的案例,與上面 CVE-2026-93144 同屬本批(2026-09-18)公告中的 BTF/allocated objects 驗證器問題。
> - **[CVE-2026-93173](https://ubuntu.com/security/CVE-2026-93173)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):`__bpf_prog_put_rcu()` 在 softirq context 呼叫 `security_bpf_prog_free()` 觸發 LSM hook——但若有 sleepable BPF LSM 程式掛在這個 hook 上,`might_fault()` 便會偵測到「在不允許睡眠的情境呼叫可能睡眠的函式」,觸發 `BUG: sleeping function called from invalid context`。修補把 `bpf_prog_free` 從 `sleepable_lsm_hooks` 清單移除,讓核心不再允許在此 hook 掛 sleepable 程式(非睡眠版本的 observer 仍可正常掛載),真正需要延遲的清理工作已經由既有的 RCU 寬限期保護,不受此限制影響。問題源自重構 `bpf_prog_alloc`/`bpf_prog_free` LSM hook 的 commit `1b67772`。修補見核心 commit [`2ce3f548cfc6`](https://github.com/torvalds/linux/commit/2ce3f548cfc6a1fe4c53479cf8a21931cdfd51d8)("bpf,lsm: Drop bpf_prog_free from sleepable_lsm_hooks")。屬於**驗證器/附掛檢查邏輯**允許不相容的 hook 附掛組合的案例,與上面 CVE-2026-80669(BPF LSM 附掛到 `void` 傳回路徑的 hook 觸發 `BUG_ON()`)同屬「hook 該限制卻沒限制附掛相容性」的同類問題。
> - **[CVE-2026-90159](https://ubuntu.com/security/CVE-2026-90159)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):cgroup BPF 的 `BPF_CGROUP_UNIX_GETPEERNAME`/`BPF_CGROUP_UNIX_GETSOCKNAME` hook 執行時**不持有 socket lock**,但掛在這兩個 hook 上的程式仍可呼叫 `bpf_setsockopt()`/`bpf_getsockopt()`,而這兩個 helper 內部會呼叫 `sock_owned_by_me()` 斷言目前執行緒持有該 socket 的鎖,於是觸發核心警告(`WARN`,非記憶體毀損,但代表 helper 在不該被呼叫的情境下被呼叫)。修補直接禁止在這兩個 hook 型別呼叫這兩個 sockopt helper。修補見核心 commit [`84473a7e1813`](https://github.com/torvalds/linux/commit/84473a7e1813a2da7b759ab1d098a84998c8d3f5)。屬於**驗證器/附掛檢查邏輯**允許不相容 hook 組合的案例,與上面 CVE-2026-93173、CVE-2026-80669 同屬「hook 該限制卻沒限制附掛相容性」的同類問題。
> - **[CVE-2026-90210](https://ubuntu.com/security/CVE-2026-90210)**(CVSS 7.8 High,2026-09-18 揭露):`bpf_trampoline_update()` 若在 `modify_fentry_multi()`/`unregister_fentry_multi()` 執行**之前**就失敗,此時 `cur_image` 其實還等於 `old_image`(尚未真的切換),但失敗處理路徑仍照常釋放 `old_image`——而 ftrace 仍持續呼叫進這塊已被釋放的 trampoline 映像,形成 Use-After-Free。修補見核心 commit [`0253073fb7d7`](https://github.com/torvalds/linux/commit/0253073fb7d79a2dd2eae9581ea16db2aef395a6)("bpf: Fix UAF in bpf_trampoline_multi_attach_free on update failure")。屬於本節分類中的 **maps/helper**(BPF trampoline/多重附掛)攻擊面,與上面 CVE-2026-89518/89516(sched_ext struct_ops 生命週期管理出錯)同屬「附掛/更新失敗路徑清理不完整」導致 UAF 的同類問題。
> - **[CVE-2026-92486](https://ubuntu.com/security/CVE-2026-92486)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):BPF 子程式呼叫遵循 `bpf_callback_t`(3 個參數)的 ABI 慣例,但核心的 task work 整合機制卻用一般的函式指標型別去呼叫這個 callback——在啟用 kCFI(kernel Control Flow Integrity,比對函式指標實際型別與呼叫端預期型別是否一致)的組態下,這種型別不一致會被 kCFI 判定為可疑的控制流轉移,直接觸發核心 Panic。修補見核心 commit [`2805abd08957`](https://github.com/torvalds/linux/commit/2805abd089576799b15092949420e3f8ba97fabd)。屬於**驗證器/核心介接層**對 BPF callback ABI 型別檢查不足的案例,是本節少見的 kCFI 相關問題。
> - **[CVE-2026-92520](https://ubuntu.com/security/CVE-2026-92520)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):BPF queue/stack map 的 `pop`/`peek` helper 在 map 為空時會把呼叫端的輸出緩衝區清零後才回傳錯誤,但另一條「取鎖失敗」(`-EBUSY`)的錯誤路徑卻沒有同樣清零就直接回傳,讓 BPF 程式能讀到這段未初始化的核心堆疊內容(資訊外洩)。修補見核心 commit [`7ac6e1ae41a0`](https://github.com/torvalds/linux/commit/7ac6e1ae41a09f1dd4baeeff1d028ae49ee01232)。屬於本節分類中的 **maps/helper**(queue/stack map)攻擊面,與上面 CVE-2026-93174 同屬本批未初始化記憶體外洩的姊妹問題。
> - **[CVE-2026-93111](https://ubuntu.com/security/CVE-2026-93111)**(CVSS 7.8 High,2026-09-18 揭露):建立 `tracing_multi` link 時沒有設定 `ftrace_managed` 標記,導致之後若在同一批函式上再附掛一個 `fentry` link,原本的 `tracing_multi` link 在釋放時可能無法被正確釋放。修補見核心 commit [`30bdd6d1384d`](https://github.com/torvalds/linux/commit/30bdd6d1384d894931f113eb595636092d8e650c)。屬於本節分類中的 **maps/helper**(BPF trampoline/ftrace 多重附掛)攻擊面,與上面 CVE-2026-90210 同屬本批 trampoline/多重附掛路徑出錯的姊妹問題。
> - **[CVE-2026-93112](https://ubuntu.com/security/CVE-2026-93112)**(CVSS 7.1 High,Ubuntu priority Medium,2026-09-18 揭露):`bpf_cpumask_populate()` 這個 kfunc 原本設計是把資料寫入一個可寫的 `struct bpf_cpumask *`,但函式簽章實際接受的是唯讀的 `struct cpumask *`——而其他唯讀 kfunc(例如取得目前線上 CPU 遮罩的介面)也會回傳這個唯讀型別,使得 BPF 程式能把一個借來的、本應唯讀的 cpumask 傳給 `bpf_cpumask_populate()` 當成可寫目的地,寫入不屬於自己的記憶體。修補收緊函式簽章,要求傳入真正可寫、由 `bpf_cpumask_create()` 配置的 `struct bpf_cpumask *`。修補見核心 commit [`8740156ad33b`](https://github.com/torvalds/linux/commit/8740156ad33be5071b588b594c55f279457f667c)。屬於本節分類中的 **maps/helper**(cpumask kfunc)攻擊面,是本節少見的「kfunc 參數型別可寫性檢查不足」案例。
> - **[CVE-2026-93127](https://ubuntu.com/security/CVE-2026-93127)**(CVSS 7.8 High,2026-09-18 揭露):`check_stack_read_fixed_off()` 把堆疊上的純量重新讀回暫存器時,若讀取指令是**帶正負號擴展**的窄讀取(`BPF_MEMSX`),會沿用原本純量的 `id`,但實際數值卻已被 `coerce_reg_to_size_sx()` 做了正負號擴展——如果同一個堆疊槽位**同時**也被一般的**零擴展**讀取(`BPF_MEM`)讀過,兩個暫存器會共用同一個 `id`、卻代表不同的執行期數值。之後只要對零擴展那個暫存器做 `if reg == const` 比較,驗證器的 `sync_linked_regs()` 就會誤用這個共用 `id`,把正負號擴展暫存器的值「精煉」成一個它在執行期根本不會有的數值,可被利用來讓驗證器誤判邊界、造成越界存取。修補見核心 commit [`2cb5f4ca695e`](https://github.com/torvalds/linux/commit/2cb5f4ca695ebe552647e5ba4aad6934d6a43bae)("bpf: Drop scalar id on sign-extending narrowing stack fills")。屬於**驗證器**(scalar/id 追蹤)層級的問題,與上面 CVE-2026-43070(`BPF_END` 未重置 id)、CVE-2026-53081(`BPF_ADD_CONST` 誤用基底 id)同屬驗證器對純量 `id` 追蹤處理出錯的同類問題家族,這次錯在窄讀取的正負號擴展與 id 共用互相衝突。
> - **[CVE-2026-93174](https://ubuntu.com/security/CVE-2026-93174)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):`copy_map_value_long()` 在複製含特殊欄位(如 spin lock、kptr)的 per-CPU map value 時,傳給 `bpf_obj_memcpy()` 的長度是原始的 `map->value_size`,而非核心配置記憶體時實際使用、向上對齊到 8 位元組的 `round_up(map->value_size, 8)`——導致 value 結尾到對齊邊界之間的 padding bytes 永遠不會被複製、初始化;而 `lookup_elem`(及其變體)用來回傳給使用者空間的暫存緩衝區又沒有以 `__GFP_ZERO` 配置,使這段殘留的核心堆積內容(padding)可被 BPF 程式讀到、外洩給使用者空間。修補見核心 commit [`7cf9cd98cf6f`](https://github.com/torvalds/linux/commit/7cf9cd98cf6f0df3befc167ca6b54c07014d71de)。屬於本節分類中的 **maps/helper**(per-CPU map value 複製路徑)攻擊面,與上面 CVE-2026-45838 同屬未初始化記憶體外洩的同類問題,只是這次錯在複製長度未對齊,而非欄位本身忘記清零。
> - **[CVE-2026-90143](https://ubuntu.com/security/CVE-2026-90143)**(CVSS 7.8 High,2026-09-18 揭露):KCM(Kernel Connection Multiplexor)的 `kcm_parse_func_strparser()` 呼叫 `bpf_prog_run_pin_on_cpu()` 執行 BPF parser 程式時,雖然阻止了 CPU 遷移,卻沒有一併建立 RCU read-side critical section——導致這段 BPF 程式若呼叫 map 操作,會觸發 `WARN_ON_ONCE(!bpf_rcu_lock_held())` 警告,代表 map 操作是在缺乏 RCU 保護的情況下執行,存在潛在的釋放後使用風險。修補在執行程式前明確持有 RCU read lock。修補見核心 commit [`b0346dd64e49`](https://github.com/torvalds/linux/commit/b0346dd64e4905291cc9c479f2e6cf1884ced4e6)("net: kcm: Hold RCU read lock while running BPF parser")。屬於本節分類中的 **maps/helper**(KCM/BPF 附掛情境缺少 RCU 保護)攻擊面。
> - **[CVE-2026-90146](https://ubuntu.com/security/CVE-2026-90146)**(CVSS 7.8 High,2026-09-18 揭露):`bpf_xdp_link_update()` 直接呼叫 `dev_xdp_install()`,跳過了 `dev_xdp_attach()`,使得原本應該在附掛/更新時一併檢查的項目(程式是否已被硬體卸載、是否已綁定其他裝置、native/generic 模式是否相符、DEVMAP/CPUMAP 限制等)完全沒有執行——使用者可以先用一般程式建立 XDP link,再透過 `BPF_LINK_UPDATE` 把它換成一個硬體卸載 (offloaded) 或裝置綁定的程式,讓它繞過檢查、被當成一般軟體路徑程式執行。修補把這些檢查搬進 `dev_xdp_install()` 這個三條路徑(`ip link set xdp`、`BPF_LINK_CREATE`、`BPF_LINK_UPDATE`)共同會走到的入口,`dev_xdp_attach()` 只保留與它自身相關的 netlink flag 檢查。修補見核心 commit [`ad27ed7d2309`](https://github.com/torvalds/linux/commit/ad27ed7d2309419a129078d781504f486b1b469a)("bpf, xdp: move offload check into dev_xdp_install()")。屬於**驗證器/附掛檢查邏輯**允許不相容組合的案例,與上面 CVE-2026-80669、CVE-2026-90159、CVE-2026-93173 同屬「附掛/更新路徑該擋卻沒擋」的同類問題,這次錯在 `BPF_LINK_UPDATE` 繞過了 XDP 附掛應有的相容性檢查。
> - **[CVE-2026-90211](https://ubuntu.com/security/CVE-2026-90211)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):s390 BPF JIT 對 arena 指標做帶 `BPF_FETCH` 的原子讀改寫 (RMW) 運算時,會轉成 `BPF_PROBE_ATOMIC` 並註冊例外表項目——但 `bpf_jit_probe_atomic_pre()` 只填入了 arena base 與 probe offset,卻沒有填入需要在錯誤發生時被清零的「fetch 目的暫存器」欄位(維持在初始化時的 -1),導致 `ex_handler_bpf()` 誤判成「沒有暫存器需要清零」。這對純寫入的 `BPF_{ADD,AND,OR,XOR}` 沒有影響,但帶 `BPF_FETCH` 的 RMW(以及 `BPF_XCHG`/`BPF_CMPXCHG`)會把舊值讀進暫存器——一旦存取未映射的 arena 分頁觸發錯誤,程式恢復執行時,該暫存器殘留的會是錯誤發生前的舊值,而非其他架構上一致保證的 0。修補改用 `bpf_atomic_load_reg()` 正確填入這個欄位。修補見核心 commit [`cc3e12330599`](https://git.kernel.org/stable/c/cc3e12330599f097f0e1f792435686ddc276f26d)。屬於 **JIT 編譯器**層級的 arena 例外處理出錯案例,與上面 CVE-2026-93045(arena 釋放路徑下界檢查缺失)同屬 arena 相關問題,但這次是架構特定(s390)的錯誤處理疏漏,而非邊界檢查。
> - **[CVE-2026-90217](https://ubuntu.com/security/CVE-2026-90217)**(CVSS 7.8 High,2026-09-18 揭露):驗證器對堆疊上的 `STACK_ITER`(iterator)槽位做狀態剪枝 (state pruning) 比對時,沒有比較指標型別本身——一個 iterator 槽位可能是 `MEM_RCU`(有 RCU 保護)或 `PTR_UNTRUSTED`(不受信任),這兩種型別在安全性上完全不同,但驗證器卻把它們視為等價狀態,可能把一條實際不安全的執行路徑,誤判成與先前已驗證過的安全路徑等價而直接剪枝略過。修補在比對 `STACK_ITER` 槽位時,一併比較指標型別是否相符。修補見核心 commit [`83608e303b95`](https://github.com/torvalds/linux/commit/83608e303b95d07afba1c15da0b5d9e513c2f15a)("bpf: Compare iterator types during state pruning")。屬於**驗證器**(狀態剪枝)層級的問題,與上面 CVE-2026-93127(narrow read 正負號擴展與 id 共用衝突)同屬「剪枝時遺漏了某個應該比較的狀態欄位」的同類問題家族。
> - **[CVE-2026-90250](https://ubuntu.com/security/CVE-2026-90250)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):`__cgroup_bpf_replace` 用 `link_update` 把一個 cgroup 上原本掛載的空程式替換成使用 `BPF_MAP_TYPE_PERCPU_CGROUP_STORAGE`、且會呼叫 `bpf_get_local_storage()` 的新程式時,沒有為這個新程式配置並指派所需的 cgroup storage——導致新程式繼承了一段未初始化的儲存指標,執行到 `bpf_get_local_storage()` 時直接對 NULL 指標解參考,造成核心 Panic。修補改為在 link update 時,若新舊程式的 cgroup storage 需求不相容就直接拒絕替換。修補見核心 commit [`3f562c537e9e`](https://github.com/torvalds/linux/commit/3f562c537e9ecf4bc5e206cfffc2cc047f1b7e94)。屬於本節分類中的 **maps/helper**(cgroup local storage)攻擊面,與上面 CVE-2026-90313(cgroup 替換失敗路徑復原不完整)同屬 cgroup BPF 程式替換路徑處理不周全的同類問題。
> - **[CVE-2026-90269](https://ubuntu.com/security/CVE-2026-90269)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):一般的 `BPF_LDX` 讀取若來源是可能出錯的指標型別(裸 `PTR_TO_BTF_ID`、`PTR_TO_BTF_ID | PTR_UNTRUSTED`、`PTR_TO_BTF_ID | MEM_ALLOC | PTR_UNTRUSTED`、`PTR_TO_MEM | MEM_RDONLY | PTR_UNTRUSTED`),驗證器會經由 `bpf_convert_ctx_accesses()` 把它改寫成帶例外表保護的 `BPF_PROBE_MEM`;但語意相同、只是多了 acquire 語意的 `BPF_LOAD_ACQ`(load-acquire)卻沒有走這條改寫路徑,JIT 因此對它產生一段**沒有例外表項目**的一般載入指令——一旦對這類指標(例如從受信任的 `task_struct` 讀出、對核心執行緒而言可能為 NULL 的 `next->mm`)做 load-acquire 觸發錯誤,就不是安全地跳到例外處理常式,而是直接核心 Panic。修補讓驗證器對 `BPF_LOAD_ACQ` 比照 `BPF_LDX` 拒絕這些來源指標型別。修補見核心 commit [`7db0a00445f1`](https://github.com/torvalds/linux/commit/7db0a00445f1a40bacfe9b747405c11cb5f10fc9)("bpf: Reject load-acquire from pointers requiring fault protection")。屬於**驗證器/JIT** 對新指令(load-acquire)未比照既有指令套用相同保護規則的案例。
> - **[CVE-2026-90299](https://ubuntu.com/security/CVE-2026-90299)**(CVSS 未公布,Ubuntu priority Medium,2026-09-18 揭露):當核心關閉 `CONFIG_FUNCTION_ERROR_INJECTION` 時,驗證器允許 sleepable tracing/LSM 程式附掛到名稱帶有 `__x64_` 等架構前綴的符號上,卻沒有先確認這個符號實際對應的是不是一個真正的核心函式——因此一個 sleepable fentry 程式可以附掛到另一個名稱恰好帶有 `__x64_` 前綴的 **BPF 程式**(而非核心函式)上,例如附掛到 `__x64_sys_nop` 這個 XDP 程式,並在其中呼叫 `bpf_copy_from_user()`。由於 XDP 程式執行環境不允許睡眠,一旦掛上的 XDP 程式被觸發(如 `ping` 一次即可),就會觸發 `BUG: sleeping function called from invalid context`。修補規定:只要附掛目標的 BTF 不是核心自身的 BTF,一律禁止 sleepable 程式附掛。修補見核心 commit [`00244bdaa423`](https://github.com/torvalds/linux/commit/00244bdaa423d93f4571f3f6854378ce3365e524)("bpf: Fix sleepable check for tracing/lsm prog")。屬於**驗證器/附掛檢查邏輯**的案例,與上面 CVE-2026-90146、CVE-2026-90159、CVE-2026-93173 同屬「附掛相容性檢查不足」的同類問題家族,這次錯在把 BPF 程式誤判成核心函式。
> - **[CVE-2026-90312](https://ubuntu.com/security/CVE-2026-90312)**(CVSS 7.8 High,2026-09-18 揭露):`check_atomic_load()` 在驗證 load-acquire 指令時,先呼叫 `check_load_mem()` 才呼叫 `atomic_ptr_type_ok()`——當指令的來源與目的是同一個暫存器(`dst_reg == src_reg`)時,`check_load_mem()` 會先把這個暫存器的型別**覆寫**成「載入後的值型別」,導致隨後執行的 `atomic_ptr_type_ok()` 已經讀不到原本的來源指標型別,無法正確拒絕 `ctx`、`pkt`、`flow_keys`、`sock` 等不該做 load-acquire 的來源型別。由於 `bpf_convert_ctx_accesses()` 不會改寫 atomic load,驗證器沒擋下的存取會直接對底層核心物件做未轉換的存取,實際讀到的是原始核心記憶體位元組,卻被驗證器當成別的型別(如 `PTR_TO_SOCK_COMMON_OR_NULL`)追蹤,形成型別混淆 (type confusion),而不只是單純的資料外洩。修補比照 `check_atomic_rmw()`,在做 load 之前先用 `check_reg_arg()`/`atomic_ptr_type_ok()` 驗證來源指標型別。修補見核心 commit [`b87803391baa`](https://github.com/torvalds/linux/commit/b87803391baa7e0bef60549d8841f12e549ad057)("bpf: Check load-acquire src ptr type before the load")。屬於**驗證器**對新指令(load-acquire)驗證順序出錯的案例,與上面 CVE-2026-90269 同屬 load-acquire 指令保護不完整的同批姊妹問題。
> - **[CVE-2026-90317](https://ubuntu.com/security/CVE-2026-90317)**(CVSS 7.8 High,2026-09-18 揭露):在 sleepable BPF 程式中,spin lock 有時是保護一個 kptr 唯一的 RCU 機制——但驗證器在程式呼叫最後一次 `bpf_spin_unlock()`、實際結束這段 RCU 保護之後,仍繼續把該指標視為有效,沒有讓它失效。這讓另一個 CPU 有機會搶先釋放這個物件,原程式後續若仍使用這個指標就會造成 Use-After-Free;文中提到一個受能力限制的執行期概念驗證,已實際在 `__bpf_get_task_stack()` 觸發了 `task_struct` 的 UAF。修補改為在解鎖前記錄程式當下是否仍處於 RCU 保護情境,只有在這次解鎖確實結束了最後一段 RCU 保護時,才讓相關指標失效(非 sleepable 程式與位於明確 RCU read-side 區段內的情況則不受影響)。修補見核心 commit [`180c7000712d`](https://github.com/torvalds/linux/commit/180c7000712db77063b3a26f4c97e7dd9038f449)("bpf: Invalidate RCU pointers after final spin unlock")。屬於**驗證器/RCU** 對指標存活期追蹤不完整的案例,是「解鎖」與「RCU 保護結束」兩件事在驗證器眼中被錯誤視為無關的少見類型。
> - **[CVE-2026-90392](https://ubuntu.com/security/CVE-2026-90392)**(CVSS 7.8 High,2026-09-18 揭露):`bpf_link_show_fdinfo()`(`/proc/<pid>/fdinfo` 讀取 BPF link 資訊)與 `bpf_link_get_info_by_fd()`(`BPF_OBJ_GET_INFO_BY_FD`)在讀取 `link->prog` 時完全沒有持有任何鎖——若此時另一執行緒透過 `bpf_link_update()` 把這個 link 換成別的程式,原本的舊程式可能被釋放,讓這兩個讀取路徑手上拿到的是已釋放記憶體的指標,形成 Use-After-Free。修補改為在 RCU 保護下讀取 `link->prog`,確保讀取期間該指標指向的物件仍然存活。修補見核心 commit [`863f3ddd0b8a`](https://github.com/torvalds/linux/commit/863f3ddd0b8ac65abfb50d3be0869268ac0e277b)("bpf: Fix potential UAF when reading bpf link info")。屬於本節分類中的 **maps/helper**(BPF link 生命週期)攻擊面,與上面 CVE-2026-90210(trampoline 更新失敗路徑 UAF)同屬「link/trampoline 併發更新時舊物件保護不足」的同類問題。
> - **[CVE-2026-93137](https://ubuntu.com/security/CVE-2026-93137)**(CVSS 7.8 High,2026-09-18 揭露):`bpf_find_vma()` 讀取目標(可能是另一個)task 的 `task->mm`,沒有先取得參照就直接呼叫 `mmap_read_trylock(mm)`——由於 `mm_struct` 不是 `SLAB_TYPESAFE_BY_RCU`,若目標 task 在這段「讀取指標」與「嘗試上鎖」之間剛好執行 `exit_mm()` 並釋放了這個 `mm_struct`,`mmap_read_trylock()` 操作到的就是已釋放的記憶體,形成 Use-After-Free。修補針對「非目前執行緒自己」的 task,改在 `task->alloc_lock` 保護下用 `spin_trylock()`(避免在 BPF 情境下阻塞)取得 `mm` 的參照,操作完成後以 `mmput_async()` 釋放;目前執行緒自身的 `task->mm` 則因為本來就穩定而維持原邏輯,中斷關閉情境與 `!CONFIG_MMU` 則直接拒絕這條路徑。修補見核心 commit [`47b079e2117a`](https://github.com/torvalds/linux/commit/47b079e2117a2ee52e21f8b72935900c702fc0b5)("bpf: Fix use-after-free on mm_struct in bpf_find_vma()")。屬於本節分類中的 **maps/helper**(task/mm iterator 相關 helper)攻擊面,與上面 CVE-2026-53084/53085(`task_vma` iterator 鎖順序/參照計數問題)、CVE-2026-74714(iterator UAF)同屬「iterator/helper 對目標 task 生命週期物件保護不足」的同類問題家族。
>
> 上述 **CVE-2026-93144/93147/93148/93173/90159/90210/92486/92520/93111/93112/93127/93174** 及本次系統性複查追加的 **CVE-2026-90143/90146/90211/90217/90250/90269/90299/90312/90317/90392/93137** 合計 23 筆,均屬同一批公告(CVE Program 原始記錄時間戳記多為 2026-09-17,Ubuntu Security Notices 揭露日期則統一為 2026-09-18,一天落差為公告流程正常延遲)。
>
> 2026-09-25 審查後補充:上一輪複查(2026-09-23)誤判 2026-09-19 之後沒有新揭露的 BPF CVE——重新交叉比對 Ubuntu Security Notices、CVE Program 官方記錄(`CVEProject/cvelistV5`)後,發現 **2026-09-24 與 2026-09-25 兩批公告其實新增了至少 14 筆 BPF 相關 CVE**,一併補上:
>
> - **[CVE-2026-93270](https://ubuntu.com/security/CVE-2026-93270)**(CVSS 未公布,Ubuntu priority Medium,2026-09-24 揭露):BPF 直譯器處理 `BPF_MOV64_PERCPU_REG`(把 per-CPU base offset 加進暫存器)這個特殊指令時,`ALU64_MOV_X` 這個 label 底下的直譯邏輯忽略了指令的 `off` 欄位——而這個 `off` 正是用來標記「這是一筆 percpu 暫存器搬移、需要另外加上 base offset」的旗標。JIT 有正確處理這個 offset,但直譯器沒有,導致當 JIT 被關閉、或程式退回直譯器執行時,percpu 暫存器實際拿到的是未加上 base offset 的原始值,後續解參考時直接觸發 page fault、核心 Panic。修補讓含有這類 percpu 定址指令的程式強制要求 `jit_required = true`,不再允許退回直譯器執行。修補見核心 commit [`57cf929a33cb`](https://git.kernel.org/stable/c/57cf929a33cb76be3f24886073ae820b4c496914)、[`7a0855e73757`](https://git.kernel.org/stable/c/7a0855e73757ee9cf25ba635a1c735018ecba742)。與上面 CVE-2026-93043/93044/93135 同屬「直譯器不支援某種 JIT-only 構造、卻仍允許 fallback」的同類問題,這次的構造是 percpu 暫存器搬移而非 gotox/arena/helper 內嵌。
> - **[CVE-2026-97944](https://ubuntu.com/security/CVE-2026-97944)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):x86 FineIBT 的函式前導碼(preamble)曾把用來存放 CFI hash 的指令從 `subl $hash, %r10d` 改成 `subl $hash, %eax`,連帶把 hash 立即值在指令中的位元組偏移量從第 7 個位元組搬到第 5 個位元組——但 `cfi_get_func_hash()`(BPF JIT 建立 struct_ops trampoline 時,用來取得被取代之樁函式 (stub) CFI hash 的函式)仍照舊從偏移量 7 讀取,读到錯誤的資料。在啟用 kCFI + FineIBT 的核心上,這會讓 struct_ops trampoline(例如 sched_ext、HID-BPF 等場景)攜帶錯誤的 CFI hash,导致核心到 BPF 的間接呼叫(如 TCP 壅塞控制初始化)在 CFI 驗證階段失敗,觸發核心 Panic。修補見核心 commit [`cd9b4595d628`](https://git.kernel.org/stable/c/cd9b4595d6283f9b8a35d7ae607508de45a351ce)、[`67cf94c30a8b`](https://git.kernel.org/stable/c/67cf94c30a8b5ebc839f282f5c5a522b35903524)、[`5a5d26f2cfe1`](https://git.kernel.org/stable/c/5a5d26f2cfe13467166219f6bf58099326912ddb)。與上面 CVE-2026-92486(task work 以一般函式指標型別呼叫 `bpf_callback_t`,觸發 kCFI Panic)同屬「BPF 與 kCFI/FineIBT 介接處型別或偏移量對不齊」的同類問題。
> - **[CVE-2026-98019](https://ubuntu.com/security/CVE-2026-98019)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):`check_func_arg()` 允許把一個可為 NULL 的 helper/kfunc 參數視為「已做過 NULL 檢查」時,沒有同步把底層的純量暫存器標記為 precise;`check_helper_call()` 對 `bpf_get_local_storage()` 第二個參數必須為零的檢查也有同樣問題。兩者都會讓驗證器的精確度傳播 (precision propagation) 鏈斷裂,導致狀態剪枝 (checkpoint pruning) 時做出錯誤判斷。修補見核心 commit [`1a3a10b030c9`](https://git.kernel.org/stable/c/1a3a10b030c96ea88868ccc060a16827c01eaa5a)。屬於**驗證器**(純量 precise 標記/精確度傳播)層級的問題,與下面 CVE-2026-98132(zero spill 精確度傳播斷裂)同屬本批精確度追蹤缺陷。
> - **[CVE-2026-98041](https://ubuntu.com/security/CVE-2026-98041)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):驗證器的 `is_branch_taken()` 靜態預測條件分支結果時,沒有區分 `BPF_JMP`(64 位元比較)與 `BPF_JMP32`(32 位元比較)——對一筆指標與零值比較的 `BPF_JMP32` 指令,`is_branch_taken()` 卻依整個 64 位元純量值去預判分支走向,而非只看低 32 位元,導致驗證器誤剪枝、放行本應拒絕的路徑。修補見核心 commit [`d16f0eafcaa6`](https://git.kernel.org/stable/c/d16f0eafcaa62e677a2b5b0dabc1f8d31dfe3d4d) 等 4 筆對應穩定分支修補。與上面 CVE-2026-43030(`regsafe()` 封包範圍比對誤判)、CVE-2026-53081(狀態剪枝時未比對 base id)同屬「驗證器狀態剪枝/分支預測遺漏應比對欄位」的同類問題家族。
> - **[CVE-2026-98043](https://ubuntu.com/security/CVE-2026-98043)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):驗證器的 `reg_not_null()` 只要指標型別的「基底」保證非 NULL,就判定該指標非 NULL——但若指標帶有**未限定範圍**的偏移量,執行期實際的偏移量仍可能讓最終位址算出來剛好是 NULL,`reg_not_null()` 卻沒有把這種情況排除。修補要求偏移量必須落在 `±BPF_MAX_VAR_OFF` 範圍內才能套用這項非 NULL 推斷。修補見核心 commit [`cb6642048739`](https://git.kernel.org/stable/c/cb6642048739bf4ee5aa09fe465511f1e87caa63)。與下面 CVE-2026-98134 同屬「驗證器對指標非 NULL 的推斷過於寬鬆」的同批姊妹問題。
> - **[CVE-2026-98049](https://ubuntu.com/security/CVE-2026-98049)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):BPF arena 指標的 32 位元 `cmpxchg`(帶 `BPF_FETCH`、經 `bpf_convert_ctx_accesses()` 轉成 `BPF_PROBE_ATOMIC`)運算結果,沒有依 BPF 語意「32 位元暫存器運算後應零擴展高 32 位元」的規則做零擴展——原因是 `is_cmpxchg_insn()` 只認得一般的 `BPF_ATOMIC` 模式,沒有把 `BPF_PROBE_ATOMIC` 也算進去,導致這條零擴展的最佳化路徑漏掉了 arena 版本。修補讓 `is_cmpxchg_insn()` 同時比對 `BPF_ATOMIC` 與 `BPF_PROBE_ATOMIC`。修補見核心 commit [`1c1b476d43a8`](https://git.kernel.org/stable/c/1c1b476d43a8b6c9bc04600369dd8cc39950d98e)。屬於本節分類中的 **maps/helper**(arena atomic)攻擊面,與上面 CVE-2026-90211(s390 arena RMW 例外處理未清零 fetch 目的暫存器)同屬 arena atomic 運算處理不完整的同類問題。
> - **[CVE-2026-98063](https://ubuntu.com/security/CVE-2026-98063)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):`btf_var_show()` 無條件呼叫 `btf_type_id_resolve()`,但這個函式會解參考 `btf->resolved_ids`——對 vmlinux BTF 這類「base BTF」而言,這個欄位在解析階段本來就不會被填,永遠是 NULL。當 BPF 程式把一個來自 vmlinux BTF 的 `BTF_KIND_VAR` 型別 ID 傳給 `bpf_snprintf_btf()` 時,就會觸發這個 NULL pointer dereference。姊妹函式 `btf_modifier_show()` 早就有對應的 NULL 防呆,唯獨 `btf_var_show()` 沒加。修補比照加上同樣的防呆。修補見核心 commit [`e472ae0d9786`](https://git.kernel.org/stable/c/e472ae0d97865e2021ee668a13eab64013aebf23)。與下面 CVE-2026-98064 同屬本批 `bpf_snprintf_btf()`/BTF show 路徑對特殊 BTF 型別（base BTF、`void`）處理不完整的姊妹問題。
> - **[CVE-2026-98064](https://ubuntu.com/security/CVE-2026-98064)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):`btf_modifier_show()` 解析完修飾詞 (modifier) 後,會無條件呼叫 `btf_type_ops(t)->show()`——但 `void` 型別根本沒有對應的 `show` 操作實作,若 BPF 程式經由 `bpf_snprintf_btf()` 傳入一個最終解析為 `const void` 的型別 ID,就會呼叫到不存在的函式指標,觸發 NULL pointer dereference。修補為找不到對應 `show` 實作的型別種類補上 `btf_df_show()` 這個預設(no-op)實作。修補見核心 commit [`717abdfd1d55`](https://git.kernel.org/stable/c/717abdfd1d559d01fdd102023f0a990fb0437240)。與上面 CVE-2026-98063 同屬同批 BTF show 路徑防呆不足的姊妹問題。
> - **[CVE-2026-98075](https://ubuntu.com/security/CVE-2026-98075)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):`jit_subprogs()`(位於 `fixups.c`)會把 `BPF_PSEUDO_FUNC` 型別的載入指令,改寫成指向真正函式位址的指令——但驗證器沒有拒絕「`BPF_PSEUDO_FUNC` 指向主程式本身(而非某個子程式)」這種情形。若搭配 `bpf_timer_set_callback()` 這類會把這個「函式參照」當成回呼(callback)登記並在之後呼叫的 helper,核心就可能對著一個從未被正確改寫成真實函式位址的值執行「呼叫」動作。修補讓驗證器直接拒絕這種指向主程式自身的 `BPF_PSEUDO_FUNC` 參照。修補見核心 commit [`118212417ba0`](https://git.kernel.org/stable/c/118212417ba0120d99f84154799f8880f07411f4)。屬於**驗證器**邏輯本身出錯的案例,與上面 CVE-2026-31413、CVE-2026-63864 同屬一類,這次錯在漏掉了對 `BPF_PSEUDO_FUNC` 參照目標的合法性檢查。
> - **[CVE-2026-98084](https://ubuntu.com/security/CVE-2026-98084)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):驗證器對 `bpf_loop()` 這類回呼型 helper 做精確度回溯 (precision backtracking) 分析時,`R1`(回呼的第一個參數,對 `bpf_loop()` 而言是目前的迭代次數)理應在多次驗證迭代間持續維持 precise 標記——但當 `bpf_loop()` 呼叫是在**第二次**驗證迭代中才被驗證到時,這段回溯邏輯會不小心把 `R1` 的 precise 標記重置掉,导致驗證器可能誤判、放行本應拒絕的程式。修補見核心 commit [`e3e4f66cc4b7`](https://git.kernel.org/stable/c/e3e4f66cc4b72333d0886ae2673c360248987889)。與上面 CVE-2026-98019 同屬本批精確度傳播/追蹤缺陷的姊妹問題,也與 CVE-2026-43009(atomic-fetch 精確度追蹤)同屬同一大類。
> - **[CVE-2026-98132](https://ubuntu.com/security/CVE-2026-98132)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):`__clean_func_state()` 在堆疊槽位上某 4 個位元組已經「半死」(half-dead,即該範圍內的資料不再被後續程式使用)時,會把該處純量零值的堆疊寫入 (spill) 降級標記成 `STACK_ZERO`——但這個降級動作會弄斷原本依附在這個純量識別碼 (id) 上的精確度傳播鏈,讓驗證器對後續使用同一識別碼的暫存器做出錯誤推斷。修補避免把純量零值的堆疊寫入降級為 `STACK_ZERO`。修補見核心 commit [`2f3536bff882`](https://git.kernel.org/stable/c/2f3536bff8823d3c5fdbbe15e17bfca696cc2b2e)。與上面 CVE-2026-98019、CVE-2026-98084 同屬本批精確度傳播鏈斷裂的同類問題,也與 CVE-2026-93127(narrow read 正負號擴展與 id 共用衝突)同屬純量 id 追蹤處理出錯的大家族。
> - **[CVE-2026-98134](https://ubuntu.com/security/CVE-2026-98134)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):`check_cond_jmp_op()` 比較兩個指標時,只要其中一個指標的型別**沒有**帶 `PTR_MAYBE_NULL` 旗標,就直接推斷它在執行期一定非 NULL——但某些型別(例如 `PTR_TO_MEM`)即使沒帶這個旗標,執行期實際上仍可能是 NULL。修補不再用「有沒有 `PTR_MAYBE_NULL` 旗標」(`type_may_be_null()`)當判斷依據,改用明確列出「哪些型別真的保證非 NULL」的白名單函式 `reg_not_null()`。修補見核心 commit [`b55c9f019e16`](https://git.kernel.org/stable/c/b55c9f019e169ed0d01394f96e172286a8b21b99)。與上面 CVE-2026-98043 同屬本批「驗證器對指標非 NULL 的推斷過於寬鬆」的姊妹問題。
> - **[CVE-2026-98149](https://ubuntu.com/security/CVE-2026-98149)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):per-CPU array/hash/cgroup-storage map 在**沒有**帶 `BPF_F_CPU`/`BPF_F_ALL_CPUS` 旗標的一般更新 (update) 路徑上,是用**邏輯 (logical) CPU 編號**去換算來源緩衝區裡對應該 CPU 資料的偏移量——但緩衝區裡的 per-CPU 資料實際上是依「目前系統上實際存在的 CPU (possible CPU)」緊密排列,而非依邏輯編號排列。當 CPU 編號不連續(例如只有 CPU 0、2、3 存在)時,這個換算方式會對錯位置讀寫,導致寫入錯誤的 per-CPU 數值,也造成對更新緩衝區的越界讀取。既有的查詢 (lookup) 路徑本來就是依 possible-CPU 順序正確走訪,只有這個更新路徑算錯。修補讓更新路徑比照查詢路徑,改用 possible-CPU 的緊密偏移量。修補見核心 commit [`75b0a6db4300`](https://git.kernel.org/linus/75b0a6db4300e4c2c9e97a0848deaa7acfb42fb7)。屬於本節分類中的 **maps/helper**(per-CPU map 緩衝區配置慣例)攻擊面,與上面 CVE-2026-53076/93174(per-CPU map value 長度/對齊算錯)同屬「per-CPU map 對緩衝區排列方式的假設出錯」的同類問題,但這次錯在稀疏 CPU 拓樸下的索引換算,而非長度對齊。
> - **[CVE-2026-98150](https://ubuntu.com/security/CVE-2026-98150)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露,與上面 CVE-2026-98149 同日公告、同屬稀疏 CPU 拓樸主題,但錯在不同函式):`bpf_map_check_op_flags()` 驗證呼叫端搭配 `BPF_F_CPU` 旗標傳入的 CPU 編號是否合法時,比對的基準是 `num_possible_cpus()`——這是一個**數量**(有幾個 CPU),而非「哪些 CPU 編號實際存在」的集合(`nr_cpu_ids`/`cpu_possible()` mask)。在 CPU 編號不連續(有「編號空洞」)的系統上,這個誤用會同時導致部分合法 CPU 編號被誤判拒絕、部分不存在的 CPU 編號卻被誤判放行,後者可能導致對 `BPF_MAP_TYPE_PERCPU_ARRAY` 之類的 map 操作觸發 page fault、核心崩潰。修補改為對照 `nr_cpu_ids` 與 `cpu_possible()` mask 驗證。修補見核心 commit [`ed54bf564ac5`](https://git.kernel.org/stable/c/ed54bf564ac52699cf4def3d0c2125d493e756f9)。與上面 CVE-2026-98149 同屬本批稀疏 CPU 拓樸問題的姊妹案例。
> - **[CVE-2026-98151](https://ubuntu.com/security/CVE-2026-98151)**(CVSS 未公布,Ubuntu priority Medium,2026-09-25 揭露):驗證器內部有一項一致性自我檢查(觸發時會回報 "REG INVARIANTS VIOLATION"),用來確保暫存器的變數偏移量 (`var_off`) 追蹤與其 32 位元子暫存器邊界彼此一致——但在處理**推測執行 (speculative execution)** 路徑(Spectre 緩解機制用來複製、分別驗證的分支路徑)時,驗證器對暫存器狀態拍照存檔 (snapshot) 的時間點,恰好落在「`dst_reg` 的 `var_off` 仍是原本的常數偏移量」與「32 位元子暫存器邊界已被清空成全範圍」這兩個步驟之間,導致存檔下來的狀態本身就自相矛盾。修補調整操作順序,確保存檔前兩者已同步更新完畢。修補見核心 commit [`150aeba624e8`](https://git.kernel.org/stable/c/150aeba624e8b7cac51c39440d7e8e1fd11de9a0) 等 4 筆對應穩定分支修補。屬於**驗證器**(推測執行路徑的暫存器狀態追蹤)層級的問題,是本節少見的「Spectre 緩解機制本身與一般狀態追蹤邏輯沒對齊」類型案例。
>
> 上述 **CVE-2026-93270**(2026-09-24 公告)與 **CVE-2026-97944/98019/98041/98043/98049/98063/98064/98075/98084/98132/98134/98149/98150/98151**(2026-09-25 公告)合計 14 筆,加上前述系統性複查追加的 **CVE-2026-64354**(2026-07-25 揭露,先前遺漏)共 15 筆,均已逐一以 Ubuntu Security Notices 個別 CVE 頁面核實(而非僅憑搜尋摘要)。經抽樣複查 CVE-2026-98xxx 鄰近編號範圍(約 20 筆),另有相當比例屬於非 BPF 子系統(如 stmmac、NTFS、nvme)問題,不排除同一批公告中仍有其他 BPF 相關 CVE 尚未逐一核實,下次複查應對 2026-09-25 這批做完整的系統性複查(參照 2026-06-24、2026-09-17/18 兩批的做法)。CVE-2026-74612(CVSS 10.0)至今仍是本節收錄清單中嚴重度最高的一筆——本批新增案例中,若計入未公布 CVSS 的個案(均為 Ubuntu priority Medium),已知數值最高者僅 CVE-2026-64354 的 CVSS 7.8,故排序無須調整。
>
