...
> **現況補充**:K8s 社群也意識到此問題,`kube-proxy` 的 **nftables 模式**已於 **1.33** 版 GA、用近似 `O(1)` 的映射結構解決了同樣的效能問題([Kubernetes 官方部落格:NFTables mode for kube-proxy](https://kubernetes.io/blog/2025/02/28/nftables-kube-proxy/))。IPVS 模式則已在 **1.35** 版被標記為棄用(deprecated),AWS 當時的 EKS 版本說明一度預告「1.36 版移除」,但 1.36 實際發布的 [CHANGELOG-1.36.md](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.36.md) 仍收錄 ipvs 後端的修正紀錄、代表程式碼並未真的移除;官方 [KEP-5495](https://github.com/kubernetes/enhancements/blob/master/keps/sig-network/5495-deprecate-ipvs-mode-in-kube-proxy/kep.yaml) 的 `disable-supported: false` 也顯示尚未進入可關閉/移除階段,目前規劃是 **1.37** 版加入 `KubeProxyIPVS` feature gate 逐步淘汰,尚未排定確切的完全移除版本,社群建議及早改用 nftables(詳見 [kubernetes/enhancements#5495](https://github.com/kubernetes/enhancements/issues/5495))。但 iptables 目前仍是上游預設模式,且 nftables/IPVS 都只解決了「Service 轉送」這一項問題,並未涵蓋 eBPF 在身分型網路策略、L7 可視性、無侵入式可觀測性上的能力——這正是 Cilium 等 eBPF 方案除了取代 kube-proxy 之外仍有價值的原因。
