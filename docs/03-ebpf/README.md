...
> **讓你在「不改核心原始碼、不裝核心模組」的前提下,安全地把自己的程式「注入」到核心內部執行。**
...
而且 eBPF 程式是**安全**的:在它被允許執行前,核心裡的**驗證器 (Verifier)** 會嚴格審查它(下一節詳述),確保它不會無窮迴圈、不會存取非法記憶體、不會搞垮系統。這正是 eBPF 與傳統核心模組最大的差異——**核心模組信任你,eBPF 不信任你,所以它先驗證你。**
...
整個生命週期:**寫 C → 編成位元組碼 → 經 `bpf()` 系統呼叫載入 → 驗證器審查 → JIT 編成原生機器碼 → 掛到掛載點 → 事件發生時執行 → 透過映射與使用者空間交換資料。**

### 2.2 程式類型 (Program Types)

eBPF 程式不是萬用的——你寫的程式必須宣告**屬於哪一種類型**,類型決定了:它能掛在哪種掛載點、能呼叫哪些**輔助函式 (Helper Functions)**、能拿到什麼樣的**上下文 (Context)**。
...
| 映射類型 | 用途 |
| --- | --- |
| `BPF_MAP_TYPE_HASH` | 雜湊表,任意鍵值查找(如:依 PID 累計次數) |
| `BPF_MAP_TYPE_ARRAY` | 陣列,索引固定 |
| `BPF_MAP_TYPE_PERCPU_HASH` / `PERCPU_ARRAY` | 每 CPU 一份,避免鎖競爭,效能極高 |
| `BPF_MAP_TYPE_PERF_EVENT_ARRAY` | 把事件串流送往使用者空間(舊式) |
| `BPF_MAP_TYPE_RINGBUF` | 環形緩衝區 (Ring Buffer),現代主流的事件串流方式 |
| `BPF_MAP_TYPE_LRU_HASH` | 自動淘汰最久未用項目的雜湊表 |

### 2.4 驗證器 (Verifier):安全的守門員

這是 eBPF「安全」的關鍵。當你透過 `bpf()` 系統呼叫載入程式時,**驗證器 (Verifier)** 會在程式真正執行**之前**,以靜態分析窮舉所有可能的執行路徑,確保:

1. **一定會結束**:不能有無窮迴圈(早期完全禁止迴圈;新核心允許**有界迴圈 (Bounded Loops)**)。
2. **不會非法存取記憶體**:每次解參考指標前,必須先檢查邊界(這就是為什麼你常被迫寫 `if (ptr + 1 > data_end) return;`)。
3. **不會洩漏核心記憶體**:不能把核心位址直接送到使用者空間。
4. **只用允許的輔助函式**:依程式類型限制可呼叫的 helper。
5. **指令數量與複雜度受限**:避免拖垮核心(早期上限 4096 條,後來放寬至百萬條等級)。

> 心法:**驗證器不是你的敵人,是你的安全帶。** 初學時被它擋下會很挫折,但它擋下的每一個錯誤,在傳統核心模組裡都可能是一次 Kernel Panic。

### 2.5 JIT 編譯:跑得跟原生一樣快

通過驗證後,**JIT (Just-In-Time) 編譯器** 會把與架構無關的 eBPF 位元組碼,翻譯成當下 CPU 的**原生機器碼 (Native Machine Code)**(x86-64、ARM64 等)。所以 eBPF 程式雖然是「動態載入的腳本」,執行效能卻**接近原生編譯的核心程式碼**,沒有直譯器的開銷。
...
| **XDP (eXpress Data Path)** | 封包剛進網卡驅動、**還沒進核心網路堆疊**時 | 網路(最高速) |
| **tc (Traffic Control)** | 核心網路堆疊的流量控制層(可看到更完整封包資訊) | 網路 |
| **cgroup hooks** | 某個 cgroup 的行程做網路 / socket 操作時 | 網路 / 安全 |
| **LSM (Linux Security Module)** | 核心安全決策點(可允許 / 拒絕操作) | 安全 |
...
**bpftrace** 是一套高階追蹤語言(語法像 awk),讓你用**一行**就完成原本要寫一大段 C 的追蹤工作。最適合探索與建立直覺。
...
**為什麼需要 CO-RE?** 早期 bcc 的做法是在**目標機器上即時編譯**——所以每台機器都要裝 LLVM/Clang 與核心標頭檔,又肥又慢。CO-RE 改變了這點:

- 編譯時透過 **BTF (BPF Type Format)** 記錄它需要的核心資料結構資訊。
- 載入時由 **libbpf** 依當前核心的 BTF 自動「重定位 (Relocation)」欄位偏移。
...
如果你的世界是雲原生(K8s 控制器、Operator、agent 多半用 Go),那麼 **[cilium/ebpf](https://github.com/cilium/ebpf)** 是純 Go 實作、無 CGO 依賴的主流函式庫,Cilium 本身就用它。
...
傳統 K8s 用 **kube-proxy + iptables** 來實作 Service 的負載平衡與轉送。問題在於:

- iptables 規則是**線性比對**的鏈結。每新增一個 Service / Endpoint,規則就增加。
- 大型叢集動輒**數千個 Service、數萬條規則**——封包每次轉送都要從頭掃這串長鏈,延遲與 CPU 隨規模**線性惡化**。
...
**Cilium** 是 CNCF 畢業專案,用 eBPF 全面重寫了 Kubernetes 的網路、安全與可觀測性層:

| Cilium 能力 | eBPF 怎麼做到 | 取代了什麼 |
| --- | --- | --- |
| **CNI(容器網路)** | 用 eBPF 在核心內處理 Pod 間封包轉送 | 傳統 bridge/overlay |
| **取代 kube-proxy** | 用 eBPF 雜湊表做 Service 負載平衡 | kube-proxy + iptables |
| **網路策略 (NetworkPolicy)** | 在 eBPF 層依**身分 (Identity)** 而非 IP 強制策略 | iptables 規則 |
...
核心 **5.8** 起引入了專屬能力 **`CAP_BPF`**,把「載入 eBPF」的權限拆出來,可搭配:

- `CAP_BPF`:基本的 eBPF 操作(建立 map、載入部分程式)。
- `CAP_PERFMON`:做追蹤 / profiling 類程式所需。
- `CAP_NET_ADMIN`:做網路類(XDP / tc)程式所需。

這讓你能給一個 agent **剛好夠用**的權限,而不是整個 root。

### 6.3 在容器 / Kubernetes 裡跑 eBPF 的注意事項

在 K8s 中部署 eBPF agent(如 Cilium、Falco、Tetragon),通常以 **DaemonSet**(每個節點一份)形式運行,並需注意:

- **特權或精細能力**:Pod 通常需要 `CAP_BPF` / `CAP_PERFMON` / `CAP_NET_ADMIN`,或在受控下使用 `privileged: true`。
- **掛載核心檔案系統**:常需把宿主的 `/sys/kernel/debug`(debugfs)、`/sys/fs/bpf`(bpffs)掛進容器,程式與 map 才能持久化與存取追蹤點。
- **eBPF 是節點層級、非命名空間化的**:eBPF 程式作用於**整個核心**,而非單一容器的命名空間。意即一個節點上的 eBPF agent **看得到該節點上所有容器**——這是它強大(全域可觀測)也需謹慎(權限邊界)之處。
...
| **《Learning eBPF》— Liz Rice** | 書籍 | 由淺入深的入門經典,O'Reilly 出版,有免費電子版可下載 |
