...
| `BPF_MAP_TYPE_PERCPU_HASH` / `PERCPU_ARRAY` | 每 CPU 各自一份獨立的值,讀寫不需加鎖、避免跨 CPU 競爭,效能極高。[核心文件:PERCPU_HASH](https://docs.kernel.org/bpf/map_hash.html) |
...
5. **指令數量與複雜度受限**:避免拖垮核心。核心 **5.2** 之前硬上限是 **4096 條指令**且複雜度上限 128K;5.2 之後改為單純的複雜度限制 `BPF_COMPLEXITY_LIMIT_INSNS`,放寬到約 **100 萬條指令**等級,程式大小本身不再是限制,純粹看驗證器能否在合理時間內窮舉完所有路徑([Linux 核心 commit:bpf: increase complexity limit and maximum program size](https://github.com/torvalds/linux/commit/c04c0d2b968ac45d6ef020316808ef6c82325a82))。
...
> **現況補充**:K8s 社群也意識到此問題,`kube-proxy` 的 **nftables 模式**已於 **1.33** 版 GA、用近似 `O(1)` 的映射結構解決了同樣的效能問題([Kubernetes 官方部落格:NFTables mode for kube-proxy](https://kubernetes.io/blog/2025/02/28/nftables-kube-proxy/))。IPVS 模式的棄用時程,**上游 (upstream) 與 AWS 文件目前不一致**,學習時務必留意:AWS 的 [EKS 1.35 版本說明](https://docs.aws.amazon.com/eks/latest/userguide/kubernetes-versions-standard.html#kubernetes-1-35)寫著「IPVS mode in kube-proxy is deprecated and will be removed in Kubernetes 1.36」;但實際查證上游 (`kubernetes/kubernetes`) 程式碼與 KEP 進度:**1.35** 版只先加上棄用警告([kubernetes/kubernetes#134539](https://github.com/kubernetes/kubernetes/pull/134539)),**1.37** 版(規劃 2026-08-26 發布,撰寫本文時尚未 GA)才加入 `KubeProxyIPVS` feature gate,其棄用訊息明確寫著「IPVS 將於 **1.40** 版預設關閉、**1.43** 版正式移除」([kubernetes/kubernetes#139397](https://github.com/kubernetes/kubernetes/pull/139397));已發布的 [Kubernetes 1.36 CHANGELOG](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.36.md) 也確實**沒有** IPVS 移除的項目。也就是說 AWS 文件上「1.36 移除」這句話目前看來尚未成真,實際移除時間點以上游 [kubernetes/enhancements#5495](https://github.com/kubernetes/enhancements/issues/5495) 追蹤進度為準;但無論哪個時程,及早改用 nftables 都是社群一致建議的方向。但 iptables 目前仍是上游預設模式,且 nftables/IPVS 都只解決了「Service 轉送」這一項問題,並未涵蓋 eBPF 在身分型網路策略、L7 可視性、無侵入式可觀測性上的能力——這正是 Cilium 等 eBPF 方案除了取代 kube-proxy 之外仍有價值的原因。
...
| **5.2** | 驗證器指令複雜度上限由舊式的 4096 條 + 128K 複雜度,放寬為單一 `BPF_COMPLEXITY_LIMIT_INSNS`(約 100 萬條等級)([commit 紀錄](https://github.com/torvalds/linux/commit/c04c0d2b968ac45d6ef020316808ef6c82325a82)) |
