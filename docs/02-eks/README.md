...
1. **先升 Control Plane**:`eksctl upgrade cluster --name ... --version <目標版本> --approve`(**一次只能升一個小版本**,例如 1.33 → 1.34,不能跳版)。若升級後發現問題,EKS 支援**版本回滾 (version rollback)**:僅限「透過 in-place 升級」升上來的叢集,可在**升級完成後 7 天內**回滾一個小版本(N→N-1);超過 7 天、或叢集是直接建立在該版本上,就只能整個重建叢集(詳見官方〈[Roll back a cluster's Kubernetes version](https://docs.aws.amazon.com/eks/latest/userguide/rollback-cluster.html)〉:「You can revert your cluster's Kubernetes control plane to the previous minor version after performing an in-place upgrade」「The rollback must be initiated within 7 days of the upgrade completing」)。
