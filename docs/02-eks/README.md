...
> **補充:2026-08 新功能——單一叢集可掛多個外部 OIDC 識別提供者(需 Kubernetes 1.32+)**。除了上面 IAM 身份(aws-auth / Access Entries)這條路,EKS 原本就支援用**外部 OIDC 相容識別提供者**(如企業內部的 IdP)做使用者驗證,但過去一個叢集只能掛一個。AWS 現在讓**執行 Kubernetes 1.32 以上**的叢集**最多可關聯 10 個**外部 OIDC 識別提供者,每個各自獨立設定與管理身份對應(identity mapping)——很適合員工、承包商、CI/CD 系統各自用不同 IdP 驗證的情境,不必再把所有人集中到同一個提供者或另外架設身份代理層。設定方式與過去相同,透過 Console 或 `AssociateIdentityProviderConfig` API(AWS CLI/SDK)逐一新增即可,且**既有的 IAM 驗證方式會繼續與所有已設定的 OIDC 提供者並存運作**,此功能不另外收費。詳見 AWS 官方 [what's new 公告(2026-08-24)](https://aws.amazon.com/about-aws/whats-new/2026/08/amazon-eks-multiple-oidc-providers/) 與 [Grant users access to Kubernetes with an external OIDC provider](https://docs.aws.amazon.com/eks/latest/userguide/authenticate-oidc-identity-provider.html)。

> **補充:2026-04 新功能——叢集治理用 IAM condition keys**。上面談的是「身份如何進叢集」,這裡是再往前一步的**組織治理**層級:誰有權**建立/改動叢集本身**。AWS 新增 7 個 IAM condition key,可掛在 `CreateCluster`、`UpdateClusterConfig`、`UpdateClusterVersion`、`AssociateEncryptionConfig` 這幾個叢集管理 API 上,搭配 IAM policy 或 AWS Organizations 的 **SCP (Service Control Policy)** 做集中治理,常見用法例如:`eks:endpointPublicAccess` / `eks:endpointPrivateAccess`(強制叢集只能開私有 API endpoint)、`eks:encryptionConfigProviderKeyArns`(強制 secrets 加密必須用指定的客戶自管 KMS key)、`eks:kubernetesVersion`(限制只能建立在組織核准的 K8s 版本上)、`eks:deletionProtection`(強制正式環境叢集開啟刪除保護)、`eks:controlPlaneScalingTier`(限制可用的 Provisioned Control Plane 級別)、`eks:zonalShiftEnabled`。這些 condition key 不另外收費、所有 EKS 可用區域皆已支援。詳見 AWS 官方 [what's new 公告(2026-04)](https://aws.amazon.com/about-aws/whats-new/2026/04/amazon-eks-iam-condition-keys/)。
...
> **補充**:2026-05 AWS 提高了一系列 IAM 配額,包含角色信任政策 (role trust policy) 長度上限從 4,096 字元提高到 8,192 字元、每帳號角色數上限從 5,000 提高到 10,000 等,詳見 [AWS 官方公告(2026-05-05)](https://aws.amazon.com/about-aws/whats-new/2026/05/aws-iam-increased-quotas/)。這讓上表「IRSA 跨叢集重用角色較不易」的限制有所緩解,但信任政策仍有長度上限,大規模多叢集情境下 Pod Identity 依然是官方建議的做法。
...
> 成本判斷:官方定價頁只列各機型的實際費率,並未直接寫死「12%」這個數字——這是社群依定價頁反推出的常見估算,可作為量級參考,但實際折扣率會因機型而異,規劃預算前建議直接查[官方定價頁](https://aws.amazon.com/eks/pricing/)核對目標機型的費率。整體來說:**一般型節點約 12% 的附加費換來不需要工程師手動管理 Karpenter 設定、Addon 升級、節點輪替 SOP**,對小型 / 中型團隊通常划算;對成本極度敏感且已有成熟 Karpenter 設定的大型艦隊,繼續使用標準 EKS + Karpenter 可能更省錢。GPU / 加速運算機型的附加費已於 2026 年 7 月大幅調降(G 系列降約 35%、P 系列與 Trainium 降約 60%),讓 Auto Mode 跑 AI/ML 工作負載的溢價明顯降低,詳見〈[Amazon EKS Auto Mode reduces GPU management fees by up to 60%](https://aws.amazon.com/about-aws/whats-new/2026/07/amazon-eks-auto-mode-gpu-price/)〉。
