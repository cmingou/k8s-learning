...
`ps` 是「某一瞬間的快照」,`top` / `htop` 則是**每隔幾秒自動刷新**的即時監控,用來看「這台機器現在忙不忙、誰在吃資源」。
...
htop 最上面那塊不是行程列表,而是**整台機器的健康概況**。最該先看懂的有三個:

**① Load average:`1.20  0.95  0.80`(三個數字)**

系統在 **過去 1 / 5 / 15 分鐘** 的平均「負載」——也就是同一時間有多少行程「**正在用 CPU,或正在排隊等 CPU / 等磁碟 I/O**」(Linux 把 R 執行中與 D 不可中斷的行程都算進去)。

- 三個數字由近到遠,用來看**趨勢**:`5.0 2.0 1.0`(遞減)是剛爆量、正在回落;`1.0 2.0 5.0`(遞增)是負載持續上升、要注意。
- **關鍵:要對照 CPU 核心數判讀**(用 `nproc` 查核心數),而不是看絕對值:

| 機器 | load average | 解讀 |
|------|--------------|------|
| 4 核 | `4.0` | 剛好滿載(每核都有一個在跑) |
| 4 核 | `8.0` | 過載,約一半行程在排隊等 CPU |
| 4 核 | `1.0` | 很輕鬆(約用 25%) |

> 口訣:**load average ÷ 核心數 ≈ 1** 就是滿載分界線。

**② Tasks:`32, 32 thr; 1 running`**

- `32` = 目前系統的**行程 (process)** 數量。
- `32 thr` = 總**執行緒 (thread)** 數——一個行程可開多條執行緒,所以這個數字通常 ≥ 行程數。
- `1 running` = 此刻真正處於 **R(執行中 / 可執行)** 狀態的有幾個。其餘幾十個大多在 **S(睡眠)**,在等事件(等網路、等鍵盤、等計時器),**並不佔 CPU**。

> 所以「有幾十個 task」不代表機器很忙——重點是 `running` 有幾個,以及 load average。

**③ Uptime**

機器**從開機到現在已連續運行多久**(例如 `up 5 days, 3:42`)。排查問題時,若 uptime 很短,代表這台機器最近可能當機重開過。

此外上半部還有 **每核一條的 CPU 使用率條**(綠=使用者程式、紅=核心)、以及 **Mem / Swp 記憶體條**(**Swap 一直在漲通常是壞訊號**,代表實體記憶體不夠了)。

> 💡 看一台機器「忙不忙」,先看 **load average** 與 CPU/記憶體使用率。之後排查 K8s 節點問題(節點 `NotReady`、Pod 被驅逐)時,SSH 進去先看這幾個數字,就是同一套思路。
