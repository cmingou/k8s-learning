...
>
> **版本進度補充**:API 持續演進中,v1.1(2024 年 5 月)把 GRPCRoute 晉升為 Standard Channel 穩定版;v1.5(實際發布於 2026 年 2 月 27 日,K8s 官方部落格公告則遲至同年 4 月才刊出)新增 `ListenerSet`、`TLSRoute`、CORS filter、client cert 驗證等能力晉升為 Standard Channel,ReferenceGrant 也在此版由 `v1beta1` 升為 `v1`;**v1.6.0**(2026 年 6 月 30 日發布)讓 **UDPRoute / TCPRoute 晉升為 GA(`v1` API,取代舊版 `v1alpha2`)**;其後的 **v1.6.1**(2026 年 7 月)與 **v1.6.2**(2026 年 9 月 3 日發布)皆屬純測試/相容性修補(例如把 `HTTPRequestRedirectFilter` 的 303/307/308 狀態碼重新歸類為 Extended conformance、修掉 TCP/UDP 加權路由的測試 flaky 問題),沒有新增 API 欄位或行為變更,**v1.6.2** 是截至目前的最新 patch 版([Release Notes](https://github.com/kubernetes-sigs/gateway-api/releases/tag/v1.6.2))。本節示範的 GatewayClass / Gateway / HTTPRoute / GRPCRoute / ReferenceGrant 在 v1.6 下皆維持穩定、可直接使用。
...
| **ReferenceGrant** | `v1`(v1.5 起,前身 `v1beta1`) | ✅ Stable (v1.5.0) | 授權跨命名空間引用 |
| **TLSRoute** | `v1` | ✅ Stable (v1.5.0) | TLS passthrough 路由規則(SNI 層) |
| **ListenerSet** | `v1` | ✅ Stable (v1.5.0) | 讓多個 Listener 掛載到同一 Gateway,支援多租戶 |
| **TCPRoute** | `v1`(取代 `v1alpha2`) | ✅ Stable (v1.6.0) | TCP 四層路由規則 |
| **UDPRoute** | `v1`(取代 `v1alpha2`) | ✅ Stable (v1.6.0) | UDP 四層路由規則 |

> Gateway API 是**獨立的 CRD**,不隨 K8s 版本內建,需額外安裝。Standard Channel 只包含**穩定版**資源,若要用仍在實驗性 (Experimental) Channel 的新特性(例如較新的 GEP 提案),需另外安裝 Experimental Channel 的 CRD,詳見 [Gateway API Releases](https://github.com/kubernetes-sigs/gateway-api/releases)。
