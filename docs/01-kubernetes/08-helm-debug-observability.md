...
> **版本現況**:[Helm v4.0.0 已於 2025-11-12 GA](https://helm.sh/blog/helm-4-released/),是目前的主線大版本(截至 2026-09 最新為 v4.3.0,於 **2026-09-09** 釋出)。Helm v3 已進入收尾階段:[官方公告](https://helm.sh/blog/helm-v3-end-of-life/)指出 v3 的最後一個功能版本 v3.22.0 於 **2026-09-09** 釋出(僅同步 Kubernetes client library 相容性,不再 backport 其他功能),之後 v3 只接受安全性修補,支援期延長至 **2027-02-10** 為止,屆時 v3 將完全停止維護;因此尚未遷移的使用者應盡快規劃升級至 v4。實際最新版請查 [Helm Releases](https://github.com/helm/helm/releases)。本章示範的指令(`upgrade`/`rollback`/`template`/`repo`)在 v3、v4 皆可直接使用、行為未變;但 `helm install` 在 v4 預設改用 **server-side apply**(v3 預設是 client-side apply,既有 Helm 3 建立的 release 升級後仍沿用原本方式),另有少數旗標改名(如 `--atomic` → `--rollback-on-failure`)與外掛式 post-renderer 等進階變更,詳見 [Helm 4 Overview — Breaking Changes](https://helm.sh/docs/overview/#breaking-changes)。
...
helm upgrade --install my-redis bitnami/redis -f my-values.yaml    # 沒裝就裝、有裝就升(冪等,CI 常用,見 helm upgrade --install 官方說明)
...
> **覆寫優先序(由低到高)**:Chart 預設 `values.yaml` → 你 `-f` 帶的檔(多個依序疊加)→ `--set` 命令列參數([官方文件](https://helm.sh/docs/helm/helm_upgrade/))。這正好對應「多環境」需求:一份共同 values + 各環境一份差異 values,就能乾淨地養出 dev/staging/prod。
...
Kustomize [自 K8s 1.14 起已內建在 `kubectl` 裡](https://kubernetes.io/docs/tasks/manage-kubernetes-objects/kustomization/),不必額外安裝:
...
>
> **版本補充:核心元件對外暴露的指標,v1.37 起支援 Native Histograms(Beta,預設啟用)**。上面提到 Prometheus 抓取的是**傳統 (classic) histogram**:預先固定幾個桶 (bucket) 邊界(例如 `0.1s`、`0.5s`、`1s`…),每個桶各自是一條獨立時間序列——桶數愈多、精細度愈高,但時間序列數量 (cardinality) 也跟著暴增,精細度與儲存成本互相拉扯,桶邊界沒抓對時(例如 1µs 與 4ms 的延遲被歸進同一桶)甚至會失去區分度。**[Native Histograms](https://kubernetes.io/docs/reference/instrumentation/native-histograms/)**([KEP-5808](https://github.com/kubernetes/enhancements/tree/master/keps/sig-instrumentation/5808-native-histogram-support))改用 Prometheus 較新的原生型別:同一個 histogram 只用**一條**時間序列、以指數桶邊界動態表示分布,不必事先決定桶數,官方 KEP 估計可讓同一個 histogram 的時間序列數量減少約 10 倍(≈90%),同時保留比固定桶更細的分布精度。此功能已於 **Kubernetes v1.37 晉升 Beta**,`NativeHistograms` 功能閘門在 **kube-apiserver、kube-scheduler、kubelet、kube-controller-manager、kube-proxy** 這些核心元件上**預設啟用**,且採**雙重輸出 (dual exposition)**:舊有的固定桶指標(`_bucket`/`_count`/`_sum`)仍不受影響、持續提供,native histogram 資料只在用戶端以 Protobuf 格式請求時才會一併附上,既有 Grafana 儀表板與告警規則不會因此中斷。要抓取並存下 native histogram 資料,scrape 端至少需要 **Prometheus 2.40**(實驗性支援),建議用 **3.0 以上**版本以取得穩定支援與可對個別 scrape job 設定 `scrape_native_histograms: true`;要停用可對個別元件下 `--feature-gates=NativeHistograms=false`(需重啟該元件)。這與上面 metrics.k8s.io 晉升 Stable 是不同的兩件事:metrics.k8s.io 服務的是 HPA/`kubectl top` 這種即時資源用量查詢,Native Histograms 影響的是**元件自身**對外暴露、給 Prometheus 抓取的效能/延遲分布指標(例如 API Server 的請求延遲分布)。詳見官方部落格〈[Kubernetes v1.37: Native Histograms Graduates to Beta](https://kubernetes.io/blog/2026/09/11/kubernetes-v1-37-native-histograms-beta/)〉與 [Native Histogram Support for Kubernetes Metrics 官方文件](https://kubernetes.io/docs/reference/instrumentation/native-histograms/)。
...
> **兩個關鍵注意事項**:
> 1. **事件預設只保留約 1 小時**(存在 etcd,由 kube-apiserver 的 [`--event-ttl`](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-apiserver/) 控制,預設值 `1h0m0s`,過期會被清掉)。所以事件是「現場急救」用的,不是長期稽核紀錄——要長期留存得靠日誌系統收走。
