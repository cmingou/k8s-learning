...
> **延伸回答(加分):** 那為什麼不乾脆放很多台?因為每筆寫入都要等「過半節點」確認落盤,**節點越多、寫入越慢**。所以實務上落在 **3(一般)或 5(要容忍 2 台同時故障,例如維護時剛好又掛一台)** 這個甜蜜點,幾乎不會用到 7 台以上。另外正式環境會把 etcd 節點**分散在不同可用區 (AZ)**,以容忍單一機房故障。
