...
> 🔑 **`fsGroup` 解決的痛點**:你以非 root (UID 1000) 跑容器,但掛進來的 PersistentVolume 裡的檔案屬於 root,結果你的程式**沒權限寫**(回想 rwx 模型:不是擁有者、不在群組,就只剩 others 的權限)。`fsGroup` 讓 K8s 在掛載時把 Volume 內容的群組改成 2000,而你的行程也屬於群組 2000,於是就能讀寫了。**這就是 Linux 檔案權限 (rwx) 與群組概念在 K8s 儲存上的直接應用。**
...
- **`allowPrivilegeEscalation: false`**:禁止行程透過 SUID 程式(回想 [Linux 3](../00-prerequisites/01-linux-basics/03-users-permissions.md) 的 SUID:執行時以檔案擁有者身分跑)等手段「取得比啟動它的父行程更多的權限」。設成 `false` 後,核心會設定 `no_new_privs` 旗標,`sudo`、`su`、SUID 的 `ping` 這類提權路徑就被堵死。
...
> 為什麼預設不是 `RuntimeDefault`?歷史包袱:K8s 為了相容舊行為,Pod 預設的 seccomp 是 `Unconfined`(不過濾)。所以這需要你**主動打開**——而下一節要講的 Restricted 等級,正是會幫你強制要求這件事。

### 6.2 AppArmor(簡介)

**AppArmor** 是另一套 Linux 安全模組,用「設定檔 (profile)」限制程式能做什麼(能讀寫哪些路徑、能用哪些能力)。在較新版 K8s 可直接用欄位指定:
...
| **Restricted**(嚴格) | **目前最佳實務** | Baseline 全部 + **強制** `runAsNonRoot`、`allowPrivilegeEscalation: false`、`drop: ["ALL"]`、`seccompProfile: RuntimeDefault` 等 | 正式環境應用的**目標等級** |
...
每個模式還能各自配一個 `-version` 標籤指定要對齊哪個版本的 PSS(如 `latest` 或 `v1.30`)。
...
如果你看舊文章,會看到 **PodSecurityPolicy (PSP)**。它是 K8s 早期的 Pod 安全機制,但因為**設計複雜、授權模型反直覺、容易設錯**,已於 **v1.21 棄用、v1.25 正式移除**。
...
> 為什麼重要?即使容器內某行程是 root,只要逃逸出去,在**主機**眼中它只是個沒權限的普通使用者——逃逸的傷害大幅降低。這是 `runAsNonRoot` 之外的**第二道保險**(就算非 root 沒做到,user namespace 還擋一層)。這是相對新的功能,各版本支援度與限制不一,先有概念。
